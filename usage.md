...
基于 ort (ONNX Runtime) 的语音识别工具集，支持 Whisper 和 SenseVoice 引擎。
...
| 参数 | 说明 | 默认值 |
|------|------|--------|
| `--model <engine>` | ASR 引擎：`whisper` 或 `sensevoice` | `sensevoice` |
...
连接 `ws://localhost:3000/ws/stream`，发送 f32 PCM（16kHz little-endian）音频块：
...
- `~/.octopus/model.json` — 模型路径配置（VAD、Whisper、SenseVoice）
