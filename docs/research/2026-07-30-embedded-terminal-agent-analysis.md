...
| **文件拖放进终端** | 双入口（HTML5 DnD 在 WKWebView 不可靠，分治）：(1) Finder OS 拖入用 Tauri `onDragDropEvent`（OS 原生 ghost）+ `formatDroppedPaths` 绝对路径；(2) 文件树内部拖拽用 pointer events + 自定义 ghost + `relPath` 相对 cwd（OSC 7 实时）+ `shellEscape`。写入 `session.paste`（bracketed paste）。Terax 用 `useTerminalFileDrop.ts`（OS 拖入）+ `quoteShellPath.ts` |
