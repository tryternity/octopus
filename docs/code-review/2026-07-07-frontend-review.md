...
> 关联: memory `imagepreview-canvas-32767-limit`（问题4 已实施，待 GUI 验证）

## 总体结论

**7 项发现全部核实成立，无误报。** 7 项均已实施（`tsc -b --force` EXIT=0 + 前端 79 单测通过）；问题 4（Canvas 超 Chromium 32767 上限）代码已修，DOM/sticky 视觉对齐待用户 GUI 验证（纯几何换算已抽 `viewportMath.ts` 单测覆盖）。
...
### 4. 长图 Canvas 超 Chromium 32767 上限（已实施，GUI 验证待确认）
...
- 待验证：DOM/sticky 视觉对齐无法纯单测（jsdom 无真实布局/canvas/sticky），须用户 GUI 验证——滚动/缩放/标注贴图/抓手平移/窗口 resize/dpr 变化（验证清单见下文「问题 4 GUI 验证清单」）。
...
| 4 | Canvas 超 32767 | ImagePreview/index.tsx + viewportMath.ts | ✅ 视口固定 canvas + sticky | tsc + 17 单测；GUI 验证待确认 |
...
## 问题 4 GUI 验证清单（代码已修，DOM/sticky 对齐须人工确认）

jsdom 无真实布局/canvas/sticky，纯单测只能锁住坐标换算（viewportMath 已覆盖）。下列场景须在真实 Tauri 窗口（CompactEditor 图片 tab）肉眼验证——**任一项错位即视为未解决，须回调**：

| # | 场景 | 验证点 |
|---|---|---|
| 1 | **长图**（natH×zoom×dpr > 32767，如滚 10+ 屏） | canvas 不再空白崩，正常显示（核心回归） |
| 2 | 短图（小于视口） | 图片居中，canvas 不超出视口 |
| 3 | 高图垂直滚动 | canvas 钉视口，只画露出区，滚动流畅无闪烁 |
| 4 | 宽图水平滚动 | canvas 钉视口左，横向露出区正确 |
| 5 | 双向滚动（宽×高图） | canvas 双向钉视口，无错位 |
| 6 | zoom 连续放大/缩小（防抖 150ms 期间） | 占位帧（原图）正常，bitmap 替换无闪 |
