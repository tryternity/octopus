...
| 当前焦点 | 按键 | 行为 |
|---------|------|------|
| 搜索框 | `Tab` | 焦点跳到 `[? 全部]` Tab 页面，选中第一个结果项 |
| 搜索框 | `↑↓` | 焦点跳到 `[? 全部]` Tab 页面的第一个/最后一个结果项 |
| 搜索框 | `Enter` | 执行 `[? 全部]` 第一个结果 |
| Tab 页结果项 | `Tab` | 循环切换：下一个 Tab 页 → ... → 回到搜索框 |
| Tab 页（任意位置） | `?` | 跳到 `[? 全部]` Tab |
| Tab 页（任意位置） | `a` | 跳到 `[a 应用]` Tab |
| Tab 页（任意位置） | `f` | 跳到 `[f 文件]` Tab |
| Tab 页（任意位置） | `>` | 跳到 `[> Shell]` Tab |
| Tab 页（任意位置） | `b` | 跳到 `[b 书签]` Tab |
| Tab 页（任意位置） | `i` | 焦点回到搜索输入框 |
| Tab 页结果项 | `↑↓` | 在结果项间上下导航 |
| Tab 页结果项 | `Enter` | 执行选中的结果项 |

**Tab 键循环顺序**：搜索框 → `[? 全部]` → `[a 应用]` → `[f 文件]` → `[> Shell]` → `[b 书签]` → 搜索框

**Tab 栏显示**：每个 Tab 上标注快捷键字母（`?`、`a`、`f`、`>`、`b`），当前激活 Tab 高亮。
