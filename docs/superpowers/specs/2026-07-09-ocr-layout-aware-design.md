...
| 段内折行 | reflow 成一整行 | markdown 语义正确，AI 输入质量最高 |
...
同一段落内的所有 Body 行合并为一行连续文本。行间空格规则（CJK 感知）：

```
last_char  = 前一行末字符
first_char = 后一行首字符
if last_char 或 first_char 是 ASCII → 插入空格
else（两者都是 CJK）→ 不插入空格
```
...
与 ASR `smart_append` 同模式（ASCII↔非 ASCII 边界检测）。标点字符（`。` `，` `.` `,`）视为 CJK 或 ASCII 按其 Unicode 属性自然判定。
...
| Body 段落 | `{reflowed_text}` | `\n\n`（空行） |

**连续列表项**：同一列表块内（连续的 ListItem，中间无 Title/Body 打断）用单 `\n` 分隔（markdown 列表语法要求）。列表块结束后空行分隔后续内容。
...
| 场景 | 处理 |
|------|------|
| 英文单词跨行断裂（`Wor`+`ld`） | 不修复，保留 `Wor ld`（修复需词库匹配，成本高、场景少） |
| 行尾标点 + 下行首字 | 标点按 Unicode 属性判定 ASCII/CJK，正常走空格规则 |
| 纯英文段落 | 每行间补空格，reflow 后为正常英文文本 |
| 数字 / 符号行 | 按其 Unicode 属性判定，数字和 ASCII 标点视为 ASCII |
