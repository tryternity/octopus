...
> - **2c-2（计划就绪 2026-06-24，待实现 + e2e）**：cloud 接入——上层 trait `StreamingPipelineEngine`（`LocalPipelineEngine` 包 StreamingRunner / `CloudPipelineEngine` 持 CloudStreamHandle 各 impl）+ cloud `close_async` 留 coordinator（不可消除，`Stage::CloudClosing` + session_id 护栏保留）；`Stage::CloudStreaming` 合并进 `Stage::Streaming`，cloud tick 迁入 `CloudPipelineEngine`。spec `2026-06-24-asr-pipeline-stage2c2-design.md`，plan `2026-06-24-asr-pipeline-stage2c2.md`。
