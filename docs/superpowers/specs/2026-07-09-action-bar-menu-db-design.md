...
| 按键 | 行为 |
|------|------|
| **↑↓** | **切换层级**：焦点在主菜单→进入子菜单（focusLayer: main→sub）；焦点在子菜单→回到主菜单（sub→main） |
| **←→** | **当前行移动**：焦点在主菜单→主菜单项之间移动；焦点在子菜单→子菜单项之间移动 |
| **Enter** | 执行当前焦点高亮项 |
| **Esc** | 焦点在子菜单→回主菜单；焦点在主菜单→关闭浮窗 |

**子菜单预览不抢焦点**：左右键在主菜单移动到 submenu 类型的项时，子菜单自动展开作为预览（让用户看到有子选项），但焦点仍在主菜单——用户必须按上下键才把焦点移入子菜单。`focusLayer` 状态（main/sub）独立于 `view` 状态（main/submenu）控制此行为。
