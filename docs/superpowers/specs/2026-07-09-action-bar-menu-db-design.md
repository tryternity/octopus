...
- **每项行内容**：图标 / 标题 / 子项计数徽标（submenu 且有子项时）/ 类型标签（色点 + 等宽大写名）/ 内置标记 / 悬浮工具栏（上移/下移/删除）
...
- `is_system=1`：删除按钮灰掉，类型 select 禁用（可改标题/图标/内容/启用，不可改 action_type）。
- **编辑表单**（内联 elevated card）：标题 / 图标（文件名或 SVG 源码 + 实时预览）/ 类型（select + 类型说明文案）/ 内容（textarea，submenu/copy 隐藏，其余按类型显示不同占位符）/ 启用开关。
...
- **新增**：Header「新增主菜单项」按钮（顶层）；submenu 节点展开后底部「新增子项」按钮（虚线占位按钮）。新增后自动进入编辑。
- 编辑后浮窗下次打开自动加载新数据
