...
**状态**: manual 模式已实现，FFT 拼接引擎已部署
**分支**: `feature/clipboard-research`

## 0. 概述

截图三期实现滚动截屏——用户框选区域后点击工具栏「滚动截图」按钮进入录制模式，在选区外手动滚动触控板/滚轮，后端 33fps 截帧 + **FFT 相位相关**亚像素位移估计 + 拼接成长图。

### 核心架构

**置顶透明覆盖层事件穿透 + 33fps 增量捕获 + FFT 相位相关亚像素拼接**

### 已放弃的方案

1. NSPanel → Trace/BPT trap 崩溃
2. auto 模式（CGEvent 模拟滚轮）→ 体验差
3. 简单 deactivate → scrollWheel 不穿透
4. NCC 模板匹配（双模板 PLL / 底部 strip）→ 整数像素误差 + 周期性假匹配
...
- macOS Window Server 识别底层窗口"可见"，保持 Chrome 持续 repainting

### 1.2 Key Window 焦点让出

- `save_frontmost_app()` 暂存前台 app PID
- `activate_prev_app()` 主线程激活前台 app 夺回 Key Focus
...
pub struct Stitcher {
    canvas: RgbaImage,
    reference_proj: Vec<f64>,   // 参考帧的 1D 垂直投影
    sticky_top: u32,
    sticky_bottom: u32,
    detected: bool,
    config: StitchConfig,
...
相位相关 `R = conj(Fa) * Fb`：
- 峰值在 `peak_idx < N/2` → `dy = peak_idx`（内容下移 = 向上滚）
- 峰值在 `peak_idx > N/2` → `dy = peak_idx - N`（内容上移 = 向下滚）
