...
候选列表**有序无分数**：候选1 得分最高（corrector 按 `bigram_score × W_CONTEXT + hit_count × W_HIT`
降序排列），但不传具体分数给 LLM。**设计决策**（不传分数的理由）：
- LLM 选词靠**上下文语义**（"去北京"vs"去背景"），corrector 分数是**统计信号**（频次+使用次数），
  维度不同——LLM 语义判断 > 统计分数。
- 传分数（如 `<候选1:0.8|候选2:0.3>`）可能干扰 LLM 机械选高分忽略语义，且增加标记复杂度
  （LLM 可能误把分数当文本输出）。
- 候选顺序已隐含"候选1 最可能"——LLM 语义无法判断时倾向第一个（corrector 的统计兜底），
  语义明确时选更合理的（LLM 的语义纠正能力发挥）。
