...
7. **z-order 遮挡 first-hit，只吸附可见窗体（v1.2，替代 v1.1 owner 过滤）**：命中 = 鼠标点处 z-order 最顶（屏幕上可见）的窗口——按 layer 降序 + 同 layer 数组顺序（CGWindowList 返回顺序 = 真实遮挡顺序）取首个 bounds 含点的窗口。被遮挡的窗口（盖它的窗口 layer 更高、或同 layer 数组更靠前）永不命中——「只检测可见窗体」的几何本质，与窗口 owner 无关。**放弃 v1.1 的 owner/frontmost 过滤**：frontmost（数组首个 layer0 非 self 窗口 owner pid）会错判——后台 always-on-top / 悬浮 layer0 窗口浮在最上时被当成 frontmost，真正可见的前台窗口被误滤，反而露出下方被遮挡窗口（用户实测："整个在可见应用下方、人眼看不到"的窗口仍被框选）。去掉 owner 过滤顺带修复 v1.1 过严 bug（部分露出、明明可见的后台窗口 v1.1 误杀）。部分露出的窗口，鼠标在可见部分命中后吸附**整窗 bounds**（窗口级吸附初衷）。**仍放弃 v1.0 设想的"激活+重截 / 单窗口截图 patch"**——前者覆盖窗污染新快照+激活 API 权限 trade-off，后者绘制管线/竞态复杂，均不如遮挡 first-hit 简单可靠。
...
1. `CGWindowListCopyWindowInfo(kCGWindowListOptionOnScreenOnly, kCGNullWindowID)` 取全部 on-screen 窗口元数据（数组顺序 = z-order，index 0 最顶；含 layer 信息）
2. **滤除**：
...
3. **命中（z-order 遮挡 first-hit，决策 7）**：候选 = bounds 含 `(gx, gy)` 的窗口；按 `layer` **降序**（数字大 = 更上层，菜单/弹窗 > 普通窗）取最上层，layer 相同则保留数组靠前者（= z-order 上层）= 鼠标点处屏幕上可见的最顶窗口。被遮挡的窗口（盖它的 layer 更高、或同 layer 数组更靠前）永不命中。
...
| 完全被遮挡的窗口（人眼看不到） | 遮挡 first-hit 命中盖它的上层窗口，被盖窗口永不命中 → 不吸附（v1.2 决策 7）|
| 部分露出的窗口 | 鼠标在可见部分 → 命中该窗口、吸附整窗 bounds；鼠标在被盖部分 → 命中盖它的上层窗口 |
| 单击落空（纯桌面） | 选区不动（=现状） |
...
- **单测（纯逻辑，核心）**：
  - `pick_top_window`：喂构造窗口列表，断言过滤（自身 PID / layer<0 / 退化 bounds / 跨屏）+ layer 降序取最上层；**遮挡 first-hit**：完全被盖窗口不命中、部分露出窗口在可见点命中（吸附整窗 bounds）、同 layer 数组顺序 = z-order
