...
- **窗口焦点策略（⚠️ 强需求，勿改错）**：全局快捷键不得激活 app、不得将 settings/compact_editor 带到前台。后端 `show()` 后**不调 `set_focus()`**（会触发 macOS app 激活）。action bar 由**前端 `window.focus()`** 获取 WebView 键盘焦点（不激活 app），用户唤出后方向键直接可用，无需鼠标点击。
