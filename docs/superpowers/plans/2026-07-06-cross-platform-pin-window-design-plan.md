...
- **执行过程与调整**：
  ...
  - **支持带标注/马赛克的贴图**：修复了贴图不含备注和马赛克的问题。原先 `pin_screenshot` 命令直接从后端 `ALL_CAPTURES` 缓存的原始截图中进行裁剪，导致未包含前端 Canvas 上的涂鸦信息。通过让前端调用 `composeAndCropBytes()` 拼接好包含备注与马赛克后的完整图片 Buffer，并使用浏览器原生的 `FileReader.readAsDataURL` 接口（替代会有性能压力的 8192 分块及 `apply` 方式，避免大图下的内存分配与堆栈溢出风险）高效转为 base64 发送给后端 `pin_screenshot`，由后端直接解码生成贴图窗口，成功高画质且高性能地实现了保留备注和马赛克的贴图功能。
  - **代码审计与安全性修复**：对三平台的原生贴图代码进行了全量安全审计。修复了 Windows 下 `CreateWindowExW`/`update_layered_window_view` 失败时的 raw 指针和 HBITMAP 资源泄漏问题，通过 `run_gdi_calls` 局部闭包模式完全杜绝了 GDI HDC/BITMAP 句柄的中间泄露可能；修正了 Windows `GetMessageW` 循环把 `-1` 错误返回值当做 `true` 的逻辑缺陷；移除了 macOS 模块内不必要的 trait 导入；将 macOS、Windows 和 Linux 底层事件回调中的 `expect()` / `unwrap()` Panic 隐患全部替换为安全的 `Option` 校验与 `Result` 模式匹配，避免在事件流中崩溃。同时实现了 Linux 下的鼠标位置锚定缩放，使三平台的用户缩放交互体验完全对称一致。
