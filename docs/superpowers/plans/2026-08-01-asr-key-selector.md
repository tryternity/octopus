...
- [x] **Step 1: config.rs — asr_shortcut default 改 OptRight**
...
- [x] **Step 2: config.rs — 删 ptt_key 字段 + default + Default impl**
...
- [x] **Step 3: config.rs — 删 polish_global_shortcut 字段 + default + Default impl**
...
- [x] **Step 4: config.rs — 删 record_mode 字段 + default + Default impl**
...
- [x] **Step 5: db.sql — asr_shortcut seed 改 + 删 3 行**
...
- [x] **Step 6: build 验证（预期有 error——下游引用 ptt_key/polish_global_shortcut/record_mode）**
...
- [x] **Step 1: setup.rs — 删 register_shortcut + register_polish 调用，register_ptt 改用 asr_shortcut**
...
- [x] **Step 2: setup.rs — register_ptt 兜底（值不合法 fallback OptRight）**
...
- [x] **Step 3: core/shortcut.rs — 删 register_shortcut 函数**
...
- [x] **Step 4: result_window.rs — 删 register_polish_global_shortcut + trigger_global_polish**

- 删 `register_polish_global_shortcut` 函数（line 463 起）
- 删 `trigger_global_polish` 函数（仅 polish handler 调用）
- 确认无其他调用点

- [x] **Step 5: ptt.rs — 注释更新**
...
- [x] **Step 6: build 验证（仍有 error——settings_commands.rs 引用）**
...
- [x] **Step 1: set_config — 删 old tuple 的 old_asr_sc/old_polish_global + 改 asr 热重载**
...
- [x] **Step 2: apply_config_value — asr_shortcut arm 加校验 + 删 polish arm**
...
- [x] **Step 3: 确认 register_shortcut 调用全删 + 删函数**
...
- [x] **Step 1: GeneralPanel.tsx — 「语音识别」行改 dropdown**
...
- [x] **Step 2: GeneralPanel.tsx — 删「识别润色」行**
...
- [x] **Step 3: locale — 删 polishShortcut + 加 PTT 键说明**
...
- [x] **Step 2: single-key-three-modes spec 不变量 #5 更新**
