...
- [x] `load_llm_model_at` 改用 `parse_model_spec`，按三分支构建 SQL：
  - `Local` → `domain='llm' AND is_local=1 AND name=?`
  - `Category` → `domain='llm' AND category=? AND name=?`
  - `NameOnly` → `domain='llm' AND name=?`（旧行为）
...
- [x] 新增 `resolve_engine_in_config(cfg, spec)` 统一解析入口（三分支）
...
- [x] `asr/src/config.rs` 测试：
  ...
  - `resolve_category_prefix_wrong_category_returns_none` / `resolve_bare_name_backward_compat`
