...
> **实施记录（2026-08-05 执行完毕，2026-08-14 归档前回写）**：11 步全部完成。**偏差**：实现从 macOS 单平台扩展为**跨平台三 hint**（macOS `org.nspasteboard.ConcealedType` / Windows `ExcludeClipboardContentFromMonitorProcessing` / Linux `x-kde-passwordManagerHint`，各自 `#[cfg]` 门控，`CONCEALED_HINTS` 常量数组）——spec 已同步跨平台演进。macOS 原方案 commit `ae5efc8d`，跨平台扩展 commit `c309b085`。
...
- [x] **Step 1: 在 `handle_clipboard_change` 开头加 ConcealedType 检测**
...
- [x] **Step 2: 编译验证（含 macOS 平台 + 跨平台编译检查）**
...
- [x] **Step 3: 跑 clipboard crate 现有测试（回归）**
...
- [x] **Step 1: 在 architecture.md 的 clipboard watcher 章节补 ConcealedType 检测说明**
