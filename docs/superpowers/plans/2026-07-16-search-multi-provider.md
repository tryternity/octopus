...
- **应用排名被书签挤出 top-10（matcher 增强）**：用户反馈"Chrome 在列表中但跌出前 10"。根因：`prefix_match` 只检查 target 整体前缀，"Google Chrome" 搜 "chrome" 命中失败 → 落 fuzzy（~500）+ 2000 加权 ≈ 2500，被书签 prefix（~5000）压下。**修复**：新增 `word_prefix_match`——按空格/标点分词后匹配任意词前缀（base 4500，介于 prefix 5000 和 pinyin 4000 之间）。"Google Chrome" → "Chrome" 词匹配 → 4495+2000=6495 稳压书签。match_score 升级为 exact > prefix > word-prefix > pinyin > fuzzy 五级。加 5 个 matcher 测试（含核心场景 `match_score_google_chrome_chrome_outranks_bookmark_fuzzy`）。
