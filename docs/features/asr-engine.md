...
| FireRed | `firered` | 离线 | FireRedASR2-AED CTC（小红书）；单 ONNX `model.int8.onnx`(740M)；中文+20方言+英 | 80-bin fbank 复用 `fbank::compute_fbank`（无 LFR，含 DC offset；pre-emphasis=0.0 保持旧行为待训练配置确认）+ CMVN 从 ONNX metadata 读 `cmvn_mean`/`cmvn_inv_stddev`；greedy CTC blank=0；vocab=8667 |
...
`paraformer.rs::compute_fbank` 是 Paraformer 私有实现（上述 5 步），而 SenseVoice-orig（`compute_fbank_features` 经 LFR→560 维）与 FireRed（纯 80-bin）共用 `fbank.rs::compute_fbank`——纯 80-bin log-fbank（hamming 窗，无 LFR）。2026-07-09 审查修复补齐对齐 kaldi_native_fbank 默认的两步预处理（此前缺它们致 SenseVoice 真实音频乱码，合成音频落在模型鲁棒区侥幸通过）：

1. **DC offset removal**——每帧 FFT 前减帧均值（**始终执行**，knf 默认 `remove_dc_offset=true`）
2. **Pre-emphasis**——`y[i]=x[i]-preemph_coeff*x[i-1]`（**参数化**：SenseVoice 传 0.97 对齐 knf 默认、am.mvn 基于含此步的特征统计；FireRed 训练 preemph 配置未确认，传 0.0 跳过、DC offset 仍生效）
