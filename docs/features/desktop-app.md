...
- **输入优先级**：显式 files 参数 > `PENDING_CONTEXT.files`（Quick Execute 热键路径）> **url（单行显式 URL——意图优先于 html，浏览器对纯文本选区也写 html flavor）** > html（剪贴板 HTML flavor）> text（直通）。混合文件夹+文件：文件夹逐个 `convert_folder`，结果以 `---` 分隔拼接。
- **URL 输入**（2026-08-18 增量，[URL spec](../superpowers/specs/2026-08-18-url-to-markdown-design.md)）：text 为**显式 URL**（files 之后、html 之前——意图优先；单行 `http://`/`https://` 开头，或 `www.` 开头自动补全 https；裸域名/IP 不识别——防普通文本误抓）→ 静态抓取**快速路径**（`octopus-convert::web::fetch_page`：Safari UA（与 WKWebView 渲染一致）+ gzip + charset 三级嗅探，15s 超时 / 20MB 上限 / 非 HTML 拒绝守卫）；转出 md < 200 字符判定 **SPA 空壳** → `ui/web_render.rs` 离屏 WKWebView 渲染 fallback 后重转（仅 macOS，超时 20s）；md 后处理绝对化链接/图片、title 命名 stem。产出走同款落盘/CompactEditor 链路；抓取/渲染失败走「转 Markdown 失败」错误 temp tab（不落半成品）。
...
- **异步执行 + 落盘**（2026-08-18 修订）：分支 `tokio::spawn` 后台执行、立即收口隐藏 ActionBar；完成后写 `~/Documents/octopus/markitdown/<源 stem>_<yyyymmdd-HHMMSS>.md`（`markitdown_output_dir` 配置可覆盖，同秒 `-N` 后缀防碰撞；⚠️ 覆盖输出目录到 `~/Documents/octopus` 之外会导致 CompactEditor 图片预览失效——assetProtocol scope 限制为 `$HOME/Documents/octopus/**`，下载的相对路径图 scope 拒绝——md 与图片本身不受影响），用 CompactEditor **file tab** 打开（`open_disk_file_in_compact_editor`，与 prompt 文件查看共用；编辑保存写回磁盘，同路径去重聚焦）；`write_output_to_clipboard` 由 item 字段控制（seed 默认 1）。失败开 CompactEditor 错误 temp tab。
- **「转 Markdown（下载图片）」第二命令**（2026-08-19，v62，[下载 spec](../superpowers/specs/2026-08-19-markdown-download-images-design.md)；同日从 base64 内嵌方案原地改向，见 spec §0）：**双命令并存**——seed id=13（同 `action_type='markdown'`，`action_data='download_images'`，icon `folder-down`，sort_order 5 排原命令后），script 分支按 action_data 传 `download: bool`；转换编排**零变化**，仅 `convert_and_save` 拿到 (path, md) 后做 **outermost 下载后处理**（URL 抓取 / HTML 选区两条远程图片来源都生效；md 有变化才重写文件）。下载 pass（`octopus-convert::web::download_images`）：远程图片 `![alt](http/https://...)` 逐张 GET 下载（10s/张、同 fetch_page UA；同 URL 只下载一次）落到 **md 同名目录** `<stem>_<ts>/`（自动创建，含时间戳天然零碰撞），md 内替换为**相对路径引用** `![alt](<目录名>/<文件名>)`——VSCode/Obsidian/Typora 等任何编辑器原生可显示（互操作）。文件名（`image_filename` 纯函数）：URL 末段去 query → unescape → sanitize（白名单同 sanitize_stem）→ 无扩展名按 MIME 补（png/jpg/gif/webp/svg）→ 冲突 `-N`。单张失败/超帽**保留原链接继续其余**，部分成功时结果头部加 `<!-- 下载图片 N/M 张 -->` 统计注释（仅 0<N<M；全部失败=原样无注释）。守卫 4 常量：单图 5MB / 累计 30MB / 最多 20 张 / 10s 每张。**预览机制**：CompactEditor 经 `assetProtocol`（scope `$HOME/Documents/octopus/**`）+ `convertFileSrc` 在 **DOM 渲染层**解析相对 src（`resolveImgSrc` 纯函数，仅非 http/data 的相对路径），**md 源与保存写回保持相对路径零污染**；**喂 AI 场景用原命令**（链接形态对 LLM 最友好）。
