...
- **「转 Markdown（内嵌图片）」第二命令**（2026-08-19，v62，[内嵌 spec](../superpowers/specs/2026-08-19-markdown-embed-images-design.md)）：**双命令并存**——seed id=13（同 `action_type='markdown'`，`action_data='embed_images'`，icon `image-plus`，sort_order 5 排原命令后），script 分支按 action_data 传 `embed: bool`；转换编排**零变化**，仅 `convert_and_save` 拿到 (path, md) 后做 **outermost 内嵌后处理**（URL 抓取 / HTML 选区两条远程图片来源都生效；md 有变化才重写文件）。内嵌 pass（`octopus-convert::web::embed_images`）：远程图片 `![alt](http/https://...)` 逐张 GET 下载（10s/张、同 fetch_page UA；同 URL 只下载一次）→ 替换为 `data:<mime>;base64,...`（MIME 双源：Content-Type 优先，fallback png/jpg/jpeg/gif/webp/svg 扩展名映射，未知保留链接）；单张失败/超帽**保留原链接继续其余**，部分成功时结果头部加 `<!-- 内嵌图片 N/M 张 -->` 统计注释（仅 0<N<M；全部失败=原样无注释）。守卫 4 常量：单图 5MB / 累计 30MB / 最多 20 张 / 10s 每张。产出自包含 md（存笔记/离线查看，CompactEditor 预览图可见）；**喂 AI 场景用原命令**（base64 是 token 毒药）。
