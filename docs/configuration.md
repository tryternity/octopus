...
> **`is_streaming` 字段**：标记 ASR 模型是否支持流式识别。`1` 表示流式（zipformer×3 / paraformer-streaming，走本地流式 partial），`0` 表示非流式（sensevoice / qwen3-asr / whisper / aliyun Fun-ASR，走 VAD 分段伪流式）。`is_streaming_engine(cfg)` = `resolve_active_engine(cfg.asr_engine).entry.is_streaming`，数据驱动、不再按 category 硬编码。
...
应用行为配置，v2+ 统一存储在 `~/.octopus/octopus.db` 的 `app_config` 表（key-value TEXT）。首次启动由 db.sql seed 默认值；旧 `config.yaml` 自动迁移到 DB 后重命名为 `.bak`。
...
> **⚠️ 迁移提示**：旧 `config.yaml` 首次启动 v2 版本时自动导入 DB 并重命名为 `config.yaml.bak`。旧字段 `polish_enabled` / `shortcut` / `polish_interval` 在迁移时自动转换为 `polish_mode` / `asr_shortcut` / `polish_min_interval`。
...
edit_shortcut: "Cmd+E"           # 进入编辑快捷键（窗口内）；保存固定 Cmd+Enter
```
