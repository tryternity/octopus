...
所有快捷键均可在「设置 → 常规 → 快捷键」中自定义，改完立即生效（热重载）。
...
| 导航项 | 配置内容 |
|--------|---------|
| **常规** | 语言、主题、快捷键、麦克风、降噪、粘贴行为、Git 同步 |
| **语音** | 识别语言、VAD 分段、润色模式与提示词、硬件加速、简繁输出 |
| **热词** | 拼音纠错开关、方言模糊规则、热词词典版本管理 |
| **模型管理** | ASR / 文本模型 / OCR / 翻译 四类模型的下载、激活、API Key 配置 |
| **剪贴板** | 监听开关、条数上限、自动清理、主题 |
| **命令面板** | Action Bar 菜单动作的增删改、Quick Execute 快捷键 |
| **智能体管理** | 外部 AI Agent（Pi / Claude Code 等）路径配置 |
