...
> 📷 **待补截图**（`images/asr-result-window.png`）：识别进行中的结果窗口（顶部精简小条 + 流式文字）
...
> 📷 **待补截图**（`images/asr-model-management.png`）：模型管理 → 语音识别 标签页
...
> 📷 **待补截图**（`images/asr-hotword-panel.png`）：热词管理面板（版本列表 + 词卡网格）
