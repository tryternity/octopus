...
| 窗口 | 用途 |
|------|------|
| `result_window` | 识别结果展示（可拖拽、多行滚动、透明无边框、置顶）。顶部悬停工具栏：鼠标移入展开，移出收起；工具精简为 7 个——**关闭**（首位，放弃内容保留 DB 记录）/ 系统设置 / 降噪模式 / 润色模式 / 立即润色 / 放大缩小 / **保存**（提交编辑恢复 ASR）。**CM6 改造（2026-07-11）**：文本区从 contentEditable div + 手写光标系统（`caret.ts` 122 行 + `CaretBlink.tsx` 49 行）替换为 CodeMirror 6 纯文本编辑器（`AsrEditor.tsx`），实现**始终可编辑 + 随说随编**——用户输入即暂停 ASR（`enter_edit_mode` → `drain_samples`，麦克风不停但丢弃），三种恢复路径：`edit_shortcut`（Cmd+Enter）/ 保存按钮 / 停止输入 2 秒自动 commit。CM6 `updateListener` 区分用户编辑（`isUserEvent`）与程序写入（流式 dispatch）：用户编辑 → 进入编辑态 + `addDirtyRange`（`iterChangedRanges` 累积 dirty 区间，排序合并）；流式写入 → 检查 `editingRef` 拦截。commit 携带 `{ text, dirtyRanges, caret?, selection? }`，后端 `commit_edit` 按 dirty ranges 劈段标 `Edited`，区间外保留原 kind（`rebuild_segments` + `segment_kind_at_offset` + `push_or_merge`）。**中插 + 选中替换**保留：非编辑态 CM6 `selectionSet` → `set_caret`（折叠选区→中插）/ `set_selection`（非折叠→选中替换）；用户选中后输入 → 走 dirty range（不走 ASR 替换）。diverted 延迟 300ms 移入 AsrEditor。全局润色入口：`polish_global_shortcut`。由 `app_config.hide_toolbar` 控制 hover 显隐。**编辑框尺寸双模式**（CSS 伪装方案）：物理窗口固定 720×480，精简态 520×116（AsrEditor 高 63px），长篇态撑满。**透明区点击穿透**：精简态小条下方透明区穿透到后方应用——Rust 后台轮询线程 `start_click_through_poller` 读全局鼠标位置判光标是否在小条矩形内。**移除的命令**：`CancelEdit` / `UpdateEditBuffer` / `exit_edit_without_commit`（始终可编辑，无显式取消）。 |
