...
- **Paraformer 流式热路径性能优化**（零拷贝，每 chunk 节省 ~400KB 堆分配）：① decoder_caches 更新用 `copy_from_slice` 复用预分配 Array3（省 16×320KB）；② encoder 输入用 `into_shape` 零拷贝 reshape（省 45KB clone）；③ `run_cif`/`run_cif_final` 用 `as_slice().unwrap()` 直接拿 `&[f32]`（省 20-40KB to_vec）；④ decoder input 键名 `in_cache_0..15` 预分配为 `cache_keys: Vec<String>`（省 16× format!）
