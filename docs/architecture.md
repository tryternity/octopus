...
| `settings_window` | 独立设置窗口（原生标题栏、800×600 可调大小、最小 640×480）。三页面侧边栏布局：识别记录（分页加载历史）/ 系统设置（17 字段表单，卡片分组 + toggle/select/number input + 生效时间徽章）/ 模型管理（占位）。单例管理：已打开则 `set_focus`。入口：工具栏设置按钮 + 托盘菜单「设置...」 |
