...
7. **Tauri 命令 async vs 同步**（2026-07-08）——Tauri 2 中同步 `fn` 命令在 UI 主线程执行，阻塞事件循环。含 DB 查询 / 硬件枚举（cpal `list_microphones`）/ CPU 密集推理（ONNX OCR）的命令必须声明为 `async fn` + `spawn_blocking`。`get_config`（5 次 DB + cpal）和 `ocr_image`（ONNX 推理）已改 async。轻量 DB 查询命令（`set_config`/`get_history`/`check_shortcut`）暂保持同步——开销极小，改 async 收益有限。`ocr_screenshot` 已是正确的 `spawn_blocking` 模式。
