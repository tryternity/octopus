...
无项目内依赖的最底层 crate，承载跨 crate 共享的基础设施：`consts`（固定路径常量：VAD 模型 / 默认 ASR 模型目录）+ `paths`（`octopus_config_home()` 返回 `~/.octopus`，三端统一）+ `config`（`AppConfig`——应用配置统一 schema）+ `db`（SQLite 嵌入式存储，含 `app_config` 表 / `models` 表 / `prompts` 表 / `clipboard_history` 表（统一存储 text/voice/ocr/image/file，吞并原 `transcriptions` 表）+ FTS5 虚表 / `image_data` 表）。DB schema 当前 v17（`clipboard_history` 新 schema：`content` + `ref_data` + `meta_info` 三层数据模型，`item_type` 枚举 `text`/`voice`/`ocr`/`image`/`file`；已废弃原 `transcriptions` 表，db.sql 不再含此表）。**开发期简化**：`init_schema` 以 db.sql 为唯一 schema 真相——`user_version >= 17` 跳过，其他（含全新库）跑 db.sql 建表+seed+yaml 导入一次性到 v17，无历史迁移链、无 DROP 兜底（开发期无旧库需兼容）。`with_db` 为公开 API 供其他 crate 调用。**⚠️ 并发约束（重入死锁）**：`with_db` 内部用 `std::sync::Mutex`（**非递归**），闭包内**严禁再调任何走 `with_db` 的函数**——同线程重入 = 第二次 `lock()` 永久等第一次释放，DB 锁被该线程永久持有。规则：**闭包内只做纯 SQL**，需读 config/meta 的在闭包外先取好再 move 进去。`image_util`（PNG 原样保存 + WebP 有损压缩 + JPEG 有损压缩，均接受 quality 参数）。
