...
- **应用上下文获取（app_context 模块，2026-07-12）**：trigger 时通过平台无障碍 API 获取选中文本的来源应用与前后文，让 AI 动作具备情境感知。`ContextProvider` trait + cfg 分发：macOS 走 NSWorkspace（前台 App pid/bundleId/name）+ AXUIElement C FFI（`AXFocusedUIElement` → `AXSelectedTextRange` + `AXValue` 切前后文 + `AXTitle` 窗口标题）；Windows/Linux 留 NullProvider stub（返回 Err 降级）。`classify_app(bundle_id)` 映射 AppKind（Editor/Terminal/Browser/Chat/Unknown）。Terminal 特例：`AXValue` 即 scrollback，取最近 50 行/2000 字作 `before`。Editor before/after 各 2000 字截断。500ms 超时返回部分结果。**失败降级铁律**：`gather()` 任何失败只记日志、`ActionBarContext` 仅含 `text`，绝不阻塞浮窗。LLM 动作（翻译/润色/摘要/解释）经 `build_enriched_text` 注入「来源/窗口/上文/下文」到 prompt；本地翻译引擎不注入（NMT 不需上下文）。AX 属性名用 `CFString::new("AXFocusedUIElement")` 构造（extern static 在 Rust 链接器不可见）。前端浮窗显示来源应用名标签。详见 [spec](superpowers/specs/2026-07-12-actionbar-app-context-design.md) + [plan](superpowers/plans/2026-07-12-actionbar-app-context.md)。
