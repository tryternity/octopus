...
**开发模式（HMR 热重载，2026-07-19 引入）：** `./run-octopus-dev.sh` 启 vite dev server (port 1420) + `cargo run` debug profile。改前端 → vite HMR 秒级推送 WebView，**不重编 Rust**。机制：`tauri.conf.json` 加 `build.devUrl: "http://localhost:1420"`，Tauri 在 debug build（`debug_assertions=true`）下自动把所有 `WebviewUrl::App(...)` 映射到 devUrl（query string 保留）；release build 完全忽略该字段，走 `frontendDist` 嵌入。`vite.config.ts` 加 `server.strictPort: true`（防 vite 静默改用 1421 致 Tauri 连不上）+ 顶层 `clearScreen: false`（保留 cargo 日志；**vite 8 起 clearScreen 从 server 子字段提升到顶层 UserConfig.clearScreen**，2026-07-19 v39 同步修复 `tsc -b` 编译错误，`defineConfig` 也从 `vite` 直接导入而非 `vitest/config`——vitest 4 重导出的 ViteUserConfig 与 vite 8 ServerOptions 类型不兼容）。`Cargo.toml` tauri 依赖启用 `devtools` feature → WebView 右键 Inspect Element 调试前端。**注意**：dev 模式（debug build）下 `#[cfg(debug_assertions)]` 代码会激活，曾经 `result_window.rs` 无条件 `window.open_devtools()` 弹 Inspector 挡住语音识别窗口，已删除（改按需右键开）。release 流程 `./run-octopus.sh` 完全不变。
...
- **URL 匹配 + Auto-Type 安全**（2026-07-19 修复 #1 #2 #3）：① eTLD+1 用 publicsuffix crate + 内嵌 Mozilla PSL（`crates/vault/data/public_suffix_list.dat`，原简化算法对 `.co.uk` 等多段 TLD 退化致钓鱼漏洞）；IP 字面量精确匹配。② `autotype_login` 加 `expected_bundle_id` 参数，注入前/后各校验前台 app；前端 VaultPicker handlePick 先 hide 浮窗再 invoke（首发版反了）。③ reprompt 保护的高敏感 cipher 在后端 `vault_autotype` 强制校验 `master_password`（首发版只在前端校验，DevTools 可绕过）。
...
- **list_ciphers 单行容错**（修复 #6）：返回 `(Vec<Cipher>, Vec<i64> 失败 id)`，单行解密失败（bit-flip / 迁移残留）只 log::warn 跳过，不再让整表 Err。
