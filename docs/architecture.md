...
| `watcher` | `ClipboardWatcher`：后台线程跑 `ClipboardWatcherContext::start_watch()`（阻塞），`on_clipboard_change` 回调检查 suppress flag → 判断类型（files > image > text 优先级，非三者则静默跳过避免 `read_text` 失败日志污染）→ 去重 → 存 DB → 通知前端 |
