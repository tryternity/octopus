...
- `compact_editor_commands.rs` + `compact_editor_window.rs`：**统一内容查看器（多 tab）**——tab 切换文本/图片/语音条目，取代独立 ImagePreview 窗口（详见 [统一查看器 spec](superpowers/specs/2026-07-04-unified-viewer-design.md)）。窗口单例 + 关窗即销毁，原生标题栏 **880×620 可调 + 记忆**（`WindowState` 存 `app_config`，开窗读记忆无记忆用默认居中；`CloseRequested` 保存——物理像素÷`scale_factor` 存逻辑像素，DPR 缩放修复 `c4eca38`）。Tab 模型 `{ key: '${source}:${itemId}', source: 'clipboard'|'transcription', itemId, itemType?: 'text'|'image', text? }`；图片 tab 嵌入 `ImagePreview` 组件（hidden 保持挂载 ≤5，超 5 替换最旧），语音 tab 只读 textarea。**6 个命令**——`open_compact_editor_tab(item_id, source?)`（写 `PENDING_TAB{item_id,source}` + 已开则 emit `compact-editor://open-tab` 推送并聚焦、未开建窗）/ `get_pending_compact_tab() -> Option<{item_id,source}>`（前端 mount take）/ `get_clipboard_item_text(item_id)`（读 content 供文本 tab 载入）/ `get_clipboard_item_type(item_id) -> 'text'|'image'`（前端据此渲染 textarea 或 ImagePreview）/ `get_transcription_text(id) -> String`（读 transcriptions 表 segments 合并文本，供语音只读 tab）/ `close_compact_editor`（关窗）。文本 tab：Ctrl+S / Cmd+↵ 经 `set_clipboard_item_text` 回写 DB + 系统剪贴板，关 tab 不删条目（仅关视图）。macOS 开窗切 Regular、关窗切回 Accessory。
- ~~`image_preview_commands.rs` + `image_preview_window.rs`~~ **已删除**（统一查看器 Task 5，`1928e62`）——独立图片预览窗口废弃，功能合并入 CompactEditor 图片 tab；`ImagePreview` 改为**组件**（`pages/ImagePreview/index.tsx`，props `imageId: number`，去掉 `get_pending_image` / `listen("image-preview://load")`，由父 tab 控制 imageId；保留 `listen("ocr-screenshot://result")` 接收截图 OCR blocks），不再经 App.tsx 路由。标注核心 `frontend/src/lib/annotation.ts` + `AnnotationSvg.tsx`（SVG overlay）不变。**性能优化（2026-07-03，归属组件）**：① 视口渲染——canvas 固定窗口大小，滚动时只从预缩放位图裁剪可见区域（不管图多大，GPU 合成恒定 ~8MB）；② 底图 canvas + SVG overlay——标注用 SVG 元素（标注变化零 canvas 操作）；③ zoom 走 `createImageBitmap` 异步预缩放（`zoomVersionRef` 防过时帧）；④ 先 thumb 再 full 渐进加载（`cancelled` 防竞态 + ResizeObserver 自动重算）；⑤ thumb→full 期间 `loadingFullRef` 门控禁止标注。详见 [perf spec](superpowers/specs/2026-07-03-image-viewer-perf-design.md)。
...
- `screenshot_commands.rs::ocr_screenshot`：截图 OCR 后端闭环——图片入库（截图历史）+ `ocr_rs` 识别 → `insert_ocr_item` → 经主线程调 `open_compact_editor_tab(image_id, None)` 开图片 tab + `open_compact_editor_tab(ocr_id, None)` 开文本 tab + `emit("clipboard://changed")` + `emit("ocr-screenshot://result", { text, blocks })`（推送 OCR 文本块给图片 tab 的 ImagePreview 组件叠加显示）；不再 write_text / update_search_text（编辑保存时才写剪贴板）。
...
| `compact_editor_window` | 统一内容查看器窗口（原生标题栏、**880×620 可调 + 记忆**、居中、min 400×320）。**多 tab**——tab 切换文本（可编辑）/ 图片（嵌入 ImagePreview 组件）/ 语音（只读）条目，tab key = `${source}:${itemId}` 全局唯一；文本 tab 标题 = 文本前 5 字 + `-` + id hex 后 5 位。工具栏（撤销/重做/字号/查找替换/清空/保存）+ 内容区 hidden 挂载，关窗即销毁。文本 tab Ctrl+S / Cmd+↵ 经 `set_clipboard_item_text` 回写 DB + 系统剪贴板；**清空后保存 = 删除条目**（调 `delete_clipboard_item`：仅剩该 tab 则关窗，否则关该 tab）；关 tab 不删条目（仅关视图）。图片 tab ≤5（hidden 保持挂载，超 5 替换最旧）。**窗口记忆**：`CloseRequested` 存位置/大小到 `app_config`（物理像素÷`scale_factor` 存逻辑像素），开窗读记忆。命令：`open_compact_editor_tab(item_id, source?)` / `get_pending_compact_tab()` / `get_clipboard_item_text(item_id)` / `get_clipboard_item_type(item_id)` / `get_transcription_text(id)` / `close_compact_editor`。单例：open 时已存在则 show+focus，否则创建；macOS 开窗切 Regular、关窗 `Destroyed` 经 `on_compact_editor_closed` 切回 Accessory（与 settings 对称）。入口：剪贴板文本「编辑」/ 图片「预览」、OCR 识别后（统一 insert_ocr_clipboard_item → openCompactEditorTab）、截图 OCR（图片 + 文本双 tab）、语音识别记录「查看」（source=transcription 只读 tab）。语音结果窗**不用**独立编辑器——改为原地尺寸双模式，见 `result_window` 行。 |
| `image_preview_window` | （已移除 2026-07-04，统一查看器 Task 5 `1928e62`）独立图片预览窗口废弃——功能合并入 `compact_editor_window` 图片 tab（嵌入 `ImagePreview` 组件）；`image_preview_window.rs` / `image_preview_commands.rs` 删除，capabilities ACL + activation `REGULAR_WINDOWS` + main.rs 命令注册一并移除，前端 App.tsx 路由删除。 |
| `image_migration` | 一次性迁移：`~/.octopus/clipboard_images/` → DB `image_data` BLOB。幂等（已存在的 hash 跳过），迁移成功后删除目录。启动时 `main.rs` setup 阶段调用。 |
| `clipboard_window` | 剪贴板历史浮窗（300×600，无边框圆角透明置顶，Alt+V 唤起——toggle 按焦点判断：失焦状态按快捷键直接 `show`+`set_focus` 激活，仅「可见且有焦点」才收起，避免 always-on-top 窗口失焦后仍 visible 导致需按两次；窗口位置记忆）。顶部标题栏（X + 「剪贴板」 + 右侧两 toggle 成组：监听开关 CircleCheck（绿圆+勾=监听中）/ CircleX（红圆+叉=已关闭）（`clipboard_enabled` 热重载，复制敏感内容前可快速暂停记录）+ Pin，`data-tauri-drag-region="deep"` + `cursor-grab`，点标题文本/空白均可拖动窗口，按钮因 clickable 元素被 drag.js 跳过、仍正常可点），搜索框 + 6 类过滤（全部/语音/文本/图片/文件/收藏，纯图标 tooltip），列表（hairline 分隔线，ASR 条目左侧 voice 色条，图片条目内联 WebP 缩略图，hover 显示操作按钮（编辑/预览/保存/OCR/打开/删除 + **收藏置末**——已收藏 amber 常显高亮，置首会遮其后 hover 按钮）（图片项「预览」Maximize2 → `openCompactEditorTab` 在 CompactEditor 开图片 tab，与双击粘贴互不冲突）；**操作栏容器 `onDoubleClick` 阻止冒泡**——连续快速点操作按钮（如删多条）的 dblclick 不冒泡到条目，不被误判为双击粘贴+隐藏浮窗（click 层 `stopPropagation` 拦不住 dblclick 这个独立事件；与左侧类型图标 dblclick 拦截对称）；**左侧类型图标单击即复制**（`copy_clipboard_item`，触效：icon 放大回弹 + 闪绿 + 「已复制」气泡 1.5s；`stopPropagation` 不触发选中、双击拦截不误触粘贴））。单击选中不关闭（点类型图标则复制、不选中），双击写剪贴板 → 隐藏浮窗 → 模拟 Cmd+V 自动粘贴（`paste_clipboard_item`，后端串起 hide `clipboard_window` + `focus_tracker.restore_focus` + `simulate_paste`）。复制/粘贴按条目类型还原真实内容——文本 `write_text`、图片从 `image_data` 读 WebP 原图转 PNG 后 `write_image`、文件解析 JSON 路径后 `write_files`（`write_item_to_clipboard` 统一分发；旧实现无视类型一律 `write_text`，导致图片粘出 blob_hash、文件粘出 JSON 字符串）。图片条目可保存到 `~/Downloads/octopus/`（自定义浮层选格式 JPEG/WebP/PNG + 质量，默认 JPEG 85%，可选保存后打开文件夹）。OCR（`ocr_image` 纯识别）→ `insert_ocr_clipboard_item` 新建 source=ocr 条目 → `openCompactEditorTab` 在精简编辑器打开绑定 tab 编辑（Ctrl+S 经 `set_clipboard_item_text` 回写）。剪贴板过滤新增 OCR 类别（全部/语音/文本/OCR/图片/文件/收藏），source=ocr 条目用 ScanText 图标。底部「管理」按钮跳转 Settings 剪贴板 tab（完整批量管理）。Settings 剪贴板/识别记录管理页手动「加载更多」分页，行操作与浮窗一致；管理页改进：全选排除收藏条目（避免收藏被批量删除）、收藏按钮置操作栏末尾（与浮窗一致）、图片条目时间后显示存储大小（高亮 sky-700）、过滤栏加 OCR 类别（ScanText 图标，与浮窗一致）。**识别记录管理页（HistoryPanel）单条文本 >200 字截断 +「……」省略**（`dde0529`，全文过长不便浏览），条目右侧「查看」按钮 → `openCompactEditorTab(id,'transcription')` 在统一查看器开只读语音 tab 看全文兜底（剪贴板管理页 ClipboardPanel 不截断）；剪贴板 + 识别记录两管理页全选 header `sticky top-0` 固定不随列表滚动（`41c35b1`）。修改命令（删除/清空/收藏/批量删除）成功后 `emit("clipboard://changed")` 广播，浮窗（`useClipboardHistory`）+ 设置页（`ClipboardPanel`）双端同步刷新。文件路径跨平台格式不同（Linux X11/Wayland = `file://` URI + 百分号编码；macOS/Windows = 已解码普通路径），显示（`formatFilePaths`）与打开（`open_file_item`）仅对 `file://` 开头解码，避免误伤含字面 `%XX` 的普通路径（Windows 剪贴板文件恒为已解码普通路径——clipboard-rs `win.rs` 走 `CF_HDROP`/`FileList` 返回普通 `C:\...` 路径，`file://` 分支不会触发，故无 `file:///C:/` 前导斜杠问题）。`open_file_item` 打开命令按平台：macOS `open` / Linux `xdg-open` / Windows `cmd /c start ""`（默认关联程序）；Windows **不用** `explorer <file>`——它只在资源管理器「定位并选中」文件、不调默认程序（旧实现用 explorer，已改 cmd start）。 |
...
- **结果窗光标定位 + 中间插入 + 选中替换**（[spec](superpowers/specs/2026-07-03-asr-cursor-insert-design.md) §1–§11）：
  ...
  - **选中替换**：拖选一段 → `invoke("set_selection", {start,end})` → `set_selection` 记录 `pending_delete` + 劈 `caret_gap` 到 start（**不立即删**，保留浏览器原生高亮）；首个非空 delta 经**两条入口**（`apply_engine_full` 流式 local / `append_segment` VadSegmented 离线 + cloud partial）到达时都 `delete_range` 真删 + `push_delta_at_caret` 插入（删旧换新）——两入口须**对称消费** `pending_delete`，漏任一即该类引擎失效（初版漏 `append_segment`，离线/cloud 引擎选中后只插不删、选中文本残留，9d4a654 修，详见 spec §11.7）。点别处 `set_caret` 清 `pending_delete` 取消；CancelEdit（`exit_edit_without_commit`）同样清（防选中→Esc 取消→残留致下次说话幽灵删除，c92955e）。前端 `onMouseUp`（选区 mouseup 才完整）按 `isCollapsed` 分流（折叠→set_caret 中插；非折叠→set_selection）。
