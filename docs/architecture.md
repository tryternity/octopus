...
- **音频采集按需启停（替代常驻，修复菜单栏麦克风指示灯常亮）**：`cpal::Stream` 所有权收归 `SharedAudioState`（`Mutex<Option<cpal::Stream>>`），不再 `std::mem::forget` 泄漏保活——`AudioRecorder` 正常 Drop，Stream 生命周期随 SharedAudioState 的 Arc（Coordinator 持有）走，进程退出时统一析构。`open()` 建流后立即 `stream.pause()`（**默认暂停**：启动即不占用麦克风、菜单栏指示灯灭）；`start()` → `stream.play()`（开捕获、指示灯亮）；`stop()` → `stream.pause()`（停捕获、指示灯灭）+ drain 采样 + 重采样 + 重置 resampler。录音期间流持续播放、回调内 `is_recording` 作冗余守卫。**已知 soundness 债**：`cpal::Stream` 为 `!Send + !Sync`，此处经 `Mutex` 装入 `unsafe impl Send/Sync` 的 SharedAudioState 并跨线程访问（setup 线程建流、Coordinator 线程 play/pause/析构）——macOS CoreAudio 设备 start/stop/drop 跨线程安全（实践不崩），但违反 cpal 的 `!Send` 契约；根治方案为把 Stream 收敛到单一宿主线程（lazy `open()` 到 Coordinator 线程，或独立 audio 线程 + channel 收 Start/Stop 指令）
- **音频初始化防闪退**：`AudioRecorder::new` / `open()` 失败（无麦克风 / 权限拒绝 / 设备占用）降级为静音占位 `SharedAudioState`（无 cpal 流 → 采样恒空 → 识别静默 → 空文本回 `Idle`），`log::error` 记录但不 `expect` panic，应用仍能进托盘、改配置后重启恢复
