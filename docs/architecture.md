...
- **搜索多 Provider 架构重构（2026-07-16，v35 迁移）**：借鉴 wox 多源广播思想，重构为 `SearchProvider` trait + `FuturesUnordered` 并发扇出。**7 个 Provider**：app/file/menu(+quicklink)/bookmark/shell/calculator/url，各声明 `matches_tab` + `uses_frequency`，`search` 契约绝不返回 Err（失败返空不拖垮整体）。`SearchEngine` 持 `Vec<Box<dyn Provider>>`，`search()` 用 `join_all`（聚合版，诊断用）/`search_streaming` 用 `FuturesUnordered`（流式版）。**流式渐进渲染**：`search_stream` Tauri 命令 emit `search://batch` 事件，每 Provider 完成立即 emit **全局累积 top-10**（per-batch boost 避免重复加权 + sort + truncate），前端 `searchStream.ts` listen + `run_id` 双重校验防串扰（batch + done 都校验）。**频次加权**：新表 `search_frequency`（v35，`score_key=source|stable_field`，7 天衰减 3000/2000/1000/500 + query 精确匹配 +500，shell/calculator/url 跳过）；执行动作时 `record_search_hit` 记录（前端传 result，后端算 key 保证一致）。**shell 修复**：裸命令/带 `>` 都处理 + 55 条内置命令补全（git status 等多词）+ `~/.zsh_history` 历史匹配（进程内 OnceLock 缓存）。**bookmark 修复**：加 Safari（plist crate，需 Full Disk Access，失败返空）+ Firefox（places.sqlite 拷临时文件只读查询）+ **Chrome/Edge 多 profile 扫描**（旧代码硬编码 `Default/Bookmarks`，多账号/迁移用户的书签常在 `Profile 1`——扫 User Data 下所有 profile 目录，跳过 Guest/System，跨 profile 按 url 去重）。**新增**：calculator（evalexpr 11，整数字面量升 Float 让 `10/4=2.5`，action_type "copy" 回车复制）+ url 检测（已知假阳性 report.pdf 接受）。新增依赖：async-trait/futures/evalexpr/plist（search crate）+ rusqlite（search crate，与 infra 同版本）。保留 `search_all`（诊断）。详见 [spec](superpowers/specs/2026-07-16-search-multi-provider-design.md) + [plan](superpowers/plans/2026-07-16-search-multi-provider.md)。
