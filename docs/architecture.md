...
**translate_engine 激活语义**（2026-07-17 模型激活重构后）：激活模型统一存 DB `models.is_enabled`（Task 2 后），不再读 app_config.translate_engine。`TranslateStrategy` 解析（`action_bar_commands.rs::resolve_translate_strategy`）：`resolve_active_engine("translate")` 取激活模型 → 按 `entry.is_local` 分流 `LocalModel { resolved }` / `CloudModel { resolved }`。未激活 / 云端缺 secret_key → **`FallbackLlm` 分支**（复用激活润色 LLM `resolve_active_engine("llm")` 兜底翻译，避免到 translate 时才报错）。引擎加载延迟到 `do_translate`（策略只决定路径不预加载）：opus-mt 按文本方向 `load_opus_mt(src, tgt)` 加载子目录、m2m100 按 `local:{model_name}` spec 加载、云端按 ResolvedEngine 字段（provider/name/source/secret_key/is_thinking）即时构造 `CloudLlmEngine::new`。

**流式翻译**（2026-07-12）：`do_translate_streaming` 按换行切分段落逐段翻译，worker 线程内 `block_on(do_translate(seg, &config))` 串行执行。**2026-07-17 双事件名隔离**（修复跨窗口泄漏）：按 `TranslateEmitTarget` 分发——`Result` target 走 `translate-progress`/`translate-done`（payload 裸 String，Result ASR 翻译用）；`CompactEditor { session_id }` target 走 `compact-editor://translate-progress`/`compact-editor://translate-done`（payload `{ sessionId, text }`，CompactEditor 翻译用），前端按 sessionId 路由到具体 tab（Map 而非单值 ref，解决竞态 + 并发错路由）。详见 [spec](superpowers/specs/2026-07-17-actionbar-launch-bugfix.md)。
...
- `clipboard_history` 表（schema v31，统一存储）：识别历史 = item_type='voice' 条目。结构化真相源为 `segments`（段 JSON `[{kind:raw|polished|edited, text}]`）+ `content`（= `finish_text()` 扁平）。meta_info JSON 存 `{engine, asr_mode, char_count, polished, polish_model, duration_ms}`。v17 废弃原 `transcriptions` 表（db.sql 不再含此表）。
...
- `models` 表：模型目录（**唯一来源**，schema v31，domain 支持 `'asr' | 'llm' | 'ocr' | 'translate'`）；**云端模型无 seed（v31 起）**，由用户通过 `CloudModelForm` 自行添加/编辑/删除；参考模型列表存 `app_config`（`category='asr_cloud_model'` + `'llm_provider'`）；`source`（`is_local=1`）= 路径标识（如 `asr/whisper-small`、`translate/opus-mt`、`ocr/PP-OCRv6-small`），相对于 `~/.octopus/models/`；`secret_key`（`is_local=1`）= **声明式下载清单 manifest JSON**（`{"<相对路径>": {"source", "sha256", "size}}`，source URL 支持 `{huggingface}` / `{github}` 模板变量，**不含 `env.` 前缀**），`secret_key`（`is_local=0`）= API Key；本地 ASR（is_local=1，13 行）+ 翻译（2 行）+ OCR（2 行，`provider='local', category='paddleocr'`）初始 manifest 由 `model_manifests.rs` 常量预填（从 HF cache 本地文件计算），首次下载后校验完整。唯一键 `UNIQUE(domain, provider, category, model_name)`
...
- `add_cloud_model` / `edit_cloud_model`（async）：LLM/Translate 保存是事务性操作——后端先调 `test_llm_connection`（POST `/chat/completions` + thinking disable），通过才写 DB；失败返回 Err 阻止保存。编辑时空 secret_key 从 DB 取真实 key 测试。`insert_cloud_model` 写 `is_available=1, is_enabled=0`（Task 1 后：可用但不自动激活，用户在管理页 switch_active_model 显式激活）。`update` / `delete` WHERE 加 `AND is_local=0` 防御性守卫。**写 DB 后调 `reload_models_config()` 刷新推理缓存**（让 `load_config` 看到新增的可用云端模型，用户激活后切换到它时引擎实例化路径能找到）。设置页列表不走缓存（`list_engines_from_db` 直查 DB），即使 reload 失败也即时反映。
- `remove_cloud_model`：物理删除（`delete_cloud_model`，WHERE `AND is_local=0`）。
...
| `action_bar_items` | AI 命令面板菜单项（自引用 parent_id 两级菜单，6 种 action_type: submenu/ai/url/script/agent/copy_path，shortcut 组合快捷键，agent 绑 adapter key，accepts 按选中类型过滤） | db.sql seed + 运行时 CRUD |
...
- **激活引擎解析（Task 2 后）**：`resolve_active_engine("asr")` 读 `ACTIVE_ENGINES` 内存缓存（启动 `load_active_engine("asr")` 填充，DB `get_active_model`）。缓存未命中 fallback 到 `load_active_engine`（重查 DB）。ASR 域无激活时 fallback 兜底引擎 `zipformer-small-ctc`（`DEFAULT_ASR_MODEL_DIR` 本地打包路径，开箱可用）；LLM/OCR/Translate 域无激活返回 Err。返回 `ResolvedEngine`（含 domain/name/provider/category:String/is_thinking/entry），category 经 `as_engine_category()` 转 EngineCategory 枚举（ASR 内部路由用）。
...
- **手编 `models` 表 / `app_config` 表需重启进程生效**（`OnceLock` 缓存，运行中不可热更新；运行时修改走 `RuntimeConfig` + `persist_*`）。DB schema `user_version` 当前为 36（开发期简化：以 db.sql 为唯一真相，增量迁移 v17→v36 链详见上方 octopus-infra 节；schema 变更直接改 db.sql + 升 user_version，删库重初始化）。
...
- **K_machine**：`~/.octopus/machine-key.enc`（AES-256-GCM 加密，file_key 由 `HKDF-SHA256(machine_id + USER)` 派生）。**原计划用 OS Keychain，实施时改本地文件**——macOS 对 adhoc 签名 binary 写 Keychain 是 session-only（重启即丢），跨平台不一致。威胁模型：仅拿到 DB（无本机文件）解不开 app_key；**不防同机同用户恶意/root 进程**（IOPlatformUUID + $USER 公开可读，详见 spec §2.5）。
...
- **错误处理**：vault crate 内部用 `anyhow`；Tauri 命令边界用 `vault_error.rs` 的 `VaultError` enum（12 个用户安全变体 + `classify(anyhow)` 启发式映射 + JSON `{code, message}` 序列化），前端 `classifyError.ts` 程序化处理。`RepromptRequired` 变体为 2026-07-19 #3 新增。
...
- **feature gate**：`octopus-desktop` 加 `vault` cargo feature（默认开），关掉后 vault 模块整体 cfg 掉（`vault_state` / `vault_commands` / `autotype` / `vault_error`）。`vault_secret_access.rs` 总是编译（云端推理热路径 chokepoint，feature off 时 `Ok(raw)`）。前端通过 `is_vault_enabled()` 运行时探针条件渲染 vault UI。
...
- **activate_app bundle_id 白名单**（修复 #10，2026-07-19）：`activate_app` 入口先 `validate_bundle_id`，char-level 校验 `[A-Za-z0-9.-]` 长度 1-256。防 AppleScript 字符串字面量注入。当前 activate_app 是 dead code，未来启用前白名单就位。`OCTOPUS_BUNDLE_ID` 常量单点定义 + `test_octopus_bundle_id_matches_tauri_config` 锁死与 tauri.conf identifier 一致。
...
基于多轮性能审查（4 个并行 agent × 2 轮，60+ 条发现）的集中优化，按 "Measure First → 核实 → 修 → 验证" 流程逐项处理。详见 [perf-batch spec](superpowers/specs/2026-07-17-perf-batch-cpu-memory.md)。
...
- **paddle-ocr NEON port**——4 个文件多个函数需手写 `std::arch::aarch64` NEON intrinsics + OCR 回归测试，详见 [backlog spec](superpowers/specs/2026-07-17-paddle-ocr-neon-port-backlog.md)。
