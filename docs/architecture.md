...
| 模块 | 职责 |
|---|---|
| `engine` | `OcrEngine`：全局单例（`OnceLock`），懒加载模型。`recognize(image_bytes)` 支持任意格式（image crate 自动检测）。模型名从 `app_config.ocr_model` 读取（默认 PP-OCRv6-small）。`instance()` 用 double-checked locking（`INIT_LOCK: Mutex<()>`）串行化首次加载、保证模型只加载一次。内部 `Mutex<Option<RapidOcr>>` 提供可变性（`run` 需 `&mut self`；`None` = idle 60s 已释放、下次 `run_ocr` 自动重载，详见下方「系统状态页」② OCR idle 释放），OCR 全局互斥保证无竞争。**超长图切分**：`recognize` 对 `height > 1600`px 的长截图按块切分（`CHUNK_HEIGHT=1280` / `CHUNK_OVERLAP=200`）逐块识别 + 按绝对 y 坐标去重（`drop_overlapped_blocks`：记已收录行最大 y 底部 `covered_until_y`，下一块中 y 中心 ≤ 该值的行落入 `CHUNK_OVERLAP` 重叠区、已被前块收录而丢弃；2026-07-07 由「文本逐字相等去重」改为坐标去重——OCR 轻微波动如 `hello` vs `hello!` 致文本不等、去重失败，且易误删天然重复行）。**全局并发互斥**：`OcrLockGuard`（`static OCR_BUSY: AtomicBool` + `compare_exchange`）做 RAII 互斥。**后处理**：`merge_same_line_blocks`（det 同行多框合并 + 间隙补空格）+ `segment_english_words`（17.7K 英文词库 `words_common.txt` 贪心分词，仅 PP-OCRv5 需要——v5 CTC 不输出英文空格；v6 CTC space token 已激活，`use_word_segmentation` 按 model_name 前缀判断跳过） |
...
- `screenshot_geometry.rs`（2026-06-12 新增）：`start_scroll_recording` 提取出的纯逻辑——坐标换算（窗口原点+CSS 偏移→全局逻辑坐标 `compute_selection_global`）、显示器命中（`find_monitor_for_point`）、物理像素裁剪（`compute_physical_crop`，含 `.max(0.0)` 跨显示器边界防御）、preview 裁剪参数（`compute_preview_crop`，消除两处重复）。所有函数不依赖 Tauri/Quartz 类型，可独立单测。
...
| `clipboard_window` | 剪贴板历史浮窗（300×600，无边框圆角透明置顶，`clipboard_shortcut` 默认 CmdOrCtrl+Shift+D 唤起——toggle 按焦点判断：失焦状态按快捷键直接 `show`+`set_focus` 激活，仅「可见且有焦点」才收起，避免 always-on-top 窗口失焦后仍 visible 导致需按两次；窗口位置记忆）。顶部标题栏（X + 「剪贴板」 + 右侧两 toggle 成组：监听开关 CircleCheck（绿圆+勾=监听中）/ CircleX（红圆+叉=已关闭）（`clipboard_enabled` 热重载，复制敏感内容前可快速暂停记录）+ Pin，`data-tauri-drag-region="deep"` + `cursor-grab`，点标题文本/空白均可拖动窗口，按钮因 clickable 元素被 drag.js 跳过、仍正常可点），搜索框 + 6 类过滤（全部/语音/文本/图片/文件/收藏，纯图标 tooltip），列表（hairline 分隔线，ASR 条目左侧 voice 色条，图片条目内联 WebP 缩略图，hover 显示操作按钮（编辑/预览/保存/OCR/打开/删除 + **收藏置末**——已收藏 amber 常显高亮，置首会遮其后 hover 按钮）（图片项「预览」Maximize2 → `openCompactEditorTab` 在 CompactEditor 开图片 tab，与双击粘贴互不冲突）；**操作栏容器 `onDoubleClick` 阻止冒泡**——连续快速点操作按钮（如删多条）的 dblclick 不冒泡到条目，不被误判为双击粘贴+隐藏浮窗（click 层 `stopPropagation` 拦不住 dblclick 这个独立事件；与左侧类型图标 dblclick 拦截对称）；**左侧类型图标单击即复制**（`copy_clipboard_item`，触效：icon 放大回弹 + 闪绿 + 「已复制」气泡 1.5s；`stopPropagation` 不触发选中、双击拦截不误触粘贴））。单击选中不关闭（点类型图标则复制、不选中），双击写剪贴板 → 隐藏浮窗 → 模拟 Cmd+V 自动粘贴（`paste_clipboard_item`，后端串起 hide `clipboard_window` + `focus_tracker.restore_focus` + `simulate_paste`）。复制/粘贴按条目类型还原真实内容——文本 `write_text`、图片从 `image_data` 读 WebP 原图转 PNG 后 `write_image`、文件解析 JSON 路径后 `write_files`（`write_item_to_clipboard` 统一分发；旧实现无视类型一律 `write_text`，导致图片粘出 blob_hash、文件粘出 JSON 字符串）。图片条目可保存到 `~/Downloads/octopus/`（自定义浮层 `SaveImagePopover` 选格式 JPEG/WebP/PNG + 质量，默认 JPEG 85%，可选保存后打开文件夹；**保存失败在浮层内显示红色原因、不闪退**——旧 catch 仅 `console.error`+瞬关造成"不知成败"；**浮层 outside-click 检测经 `triggerRef` 忽略自身触发按钮**，避免 `mousedown` 关闭与 `click` toggle 时序冲突致"再次点 Download 关不掉、只闪烁"；ClipboardItem / ClipboardPanel 两调用点同款修复）。OCR（`ocr_image` 纯识别）→ `insert_ocr_clipboard_item` 新建 source=ocr 条目 → `openCompactEditorTab` 在精简编辑器打开绑定 tab 编辑（Ctrl+S 经 `set_clipboard_item_text` 回写）。剪贴板过滤新增 OCR 类别（全部/语音/文本/OCR/图片/文件/收藏），source=ocr 条目用 ScanText 图标。底部「管理」按钮跳转 Settings 剪贴板 tab（完整批量管理）。Settings 剪贴板/识别记录管理页手动「加载更多」分页，行操作与浮窗一致（2026-07-05 补全编辑+预览按钮——此前管理页 ClipboardRow 缺这两个，文本/语音/OCR 类无编辑入口、图片无预览入口；现统一经 `openCompactEditorTab` 开 compact editor tab）；管理页改进：全选排除收藏条目（避免收藏被批量删除）、收藏按钮置操作栏末尾（与浮窗一致）、图片条目时间后显示存储大小（高亮 sky-700）、过滤栏加 OCR 类别（ScanText 图标，与浮窗一致）。**Settings ClipboardPanel 设计重构（2026-07-05，frontend-design）**：filter 分组（主分类 | 内容类型 | 状态，竖线分隔）替代平铺等权按钮；每行 3px 左缘类型色条（voice=amber 渐变暗示声波，其余类型色低饱和），滚动时形成色彩编码档案视图；voice 行作为 app 签名主角——略大字号（13px vs 12px）+ font-medium + amber 渐变左缘；选中态强化（左缘变 stone-900 + bg-stone-100 + accent-stone-900 checkbox）；空状态加图标 + 引导文案；搜索加 focus 边框 + 清除按钮；加载更多升为带边框按钮。**识别记录管理页（HistoryPanel）单条文本 >200 字截断 +「……」省略**（`dde0529`，全文过长不便浏览），条目右侧「查看」按钮 → `openCompactEditorTab(id,'transcription')` 在统一查看器开只读语音 tab 看全文兜底（剪贴板管理页 ClipboardPanel 不截断）；剪贴板 + 识别记录两管理页全选 header `sticky top-0` 固定不随列表滚动（`41c35b1`）。修改命令（删除/清空/收藏/批量删除）成功后 `emit("clipboard://changed")` 广播，浮窗（`useClipboardHistory`）+ 设置页（`ClipboardPanel`）双端同步刷新。文件路径跨平台格式不同（Linux X11/Wayland = `file://` URI + 百分号编码；macOS/Windows = 已解码普通路径），显示（`formatFilePaths`——取每路径末两段缩略，**同时按 `/` 与 `\` 切割兼容 Windows 反斜杠普通路径**；旧实现仅 `split("/")` 对 `C:\…` 不切割致整段溢出）与打开（`open_file_item`——file 条目路径存 `ref_data`、`content` 为空，须读 `ref_data` 解析；旧实现误读 `content` 致 `serde_json::from_str("")` 全平台失效）仅对 `file://` 开头解码，避免误伤含字面 `%XX` 的普通路径（Windows 剪贴板文件恒为已解码普通路径——clipboard-rs `win.rs` 走 `CF_HDROP`/`FileList` 返回普通 `C:\...` 路径，`file://` 分支不会触发，故无 `file:///C:/` 前导斜杠问题）。`open_file_item` 打开命令按平台：macOS `open` / Linux `xdg-open` / Windows `cmd /c start ""`（默认关联程序）；Windows **不用** `explorer <file>`——它只在资源管理器「定位并选中」文件、不调默认程序（旧实现用 explorer，已改 cmd start）。 |
...
8. **物理/逻辑坐标转换**（⚠️ 关键，反复踩坑 5+ 次）——macOS 下所有获取屏幕坐标的 API（`CGEvent::location()`、`Monitor::position()`、`Monitor::size()`、`NSWindow.frame`）返回的都是**物理像素**（Retina 下如 2880×1800），而 Tauri `LogicalPosition` / `LogicalSize` 是**逻辑像素**（1440×900）。两者比例 = `scale_factor()`（Retina=2.0）。**任何坐标/尺寸比较或窗口定位都必须统一到逻辑像素**——物理值 ÷ `scale_factor()`。`crates/desktop/src/action_bar_commands.rs::get_mouse_position` 和 `crates/desktop/src/compact_editor_window.rs::on_compact_editor_save_state` 均已修复。`window_position.rs::is_position_visible` 也修了（Monitor position 未除 scale）。
9. **AI 命令面板**（2026-07-08）——`action_bar_window` 迷你浮窗，全局热键触发。选中文本→模拟 Cmd+C→弹出两级菜单（AI/翻译/搜索/网页 + AI 子菜单润色/摘要/解释 + 搜索子菜单 Google/百度/Bing）→AI 结果打开 CompactEditor 临时 tab 展示（不写 DB）。热键触发前隐藏常规窗口（防 Regular 激活策略把 settings 带到前台）。trigger 在后台线程执行（200ms sleep 不阻塞主线程）。capabilities 白名单必须包含 `action_bar_window`（否则 listen/invoke 被拒）。
...
- **VAD 分段切分策略**（`VadSegmentedPipeline::run_tick`，pipeline.rs，2c-3 收编自原 coordinator；完整双 VAD 架构与不变量详见 [spec](superpowers/specs/2026-07-08-vad-segmented-pipeline-design.md)）：静音边界切分（主）+ 连续超时强制切断（兜底）
  ...
  - 强制切断：音频缓冲达 `SEGMENT_DURATION_S`（20s 常量）→ 强制切断，**保留末尾 200ms（常量 `SEGMENT_OVERLAP_MS`）作下一段 overlap**（语句被硬切，需重叠保连贯）。**`force_cut` 不门控 `has_speech`**（2026-07-07 修）：detect_vad LSTM 漂移致 `has_speech` 卡 false 时，原 `&& has_speech` 会令 force_cut 永不触发 → buffer 无限堆积不吐字；解绑后达上限必切，由 `filter_vad`（每段 reset、不受漂移污染）独立兜底判定有无语音（无语音则不 spawn 但 buffer 已清，防堆积）。`segment_duration` / `segment_overlap` 原为 config 字段，因属实现细节（用户不可感知）已改为常量
...
- `clipboard_history` 表（schema v18，统一存储）：识别历史 = item_type='voice' 条目。结构化真相源为 `segments`（段 JSON `[{kind:raw|polished|edited, text}]`）+ `content`（= `finish_text()` 扁平）。meta_info JSON 存 `{engine, asr_mode, char_count, polished, polish_model, duration_ms}`。v17 废弃原 `transcriptions` 表（db.sql 不再含此表）。
...
- `crates/desktop/src/system_status_commands.rs`：
  ...
  - `ModelMemoryRegistry`（`Arc<ModelMemoryRegistry>`，sampler 持有）：模型内存估算表，`record_once(id, bytes)` 仅首次写入不覆盖（防 ort arena 复用导致后续差值偏低/为负）。
- **模型加载埋点（依赖反转）**：`crates/infra/src/model_probe.rs` 提供全局 `set_probe(ProbeFn)` / `probe(LoadPhase::{Before,After,Unload}, id)`，未注入时 no-op（infra 不依赖 sysinfo/desktop）。asr-local（`load_engine_into_cache` cache miss 段、`SileroVad::new` cache miss 段）与 ocr（`OcrEngine::instance` 加载段）在加载前后调 `probe`，id 形如 `"asr:<name>"` / `"vad:silero"` / `"ocr:<name>"`。desktop `start()` 注入闭包：Before 读进程内存（`read_self_probe_memory`：macOS phys_footprint、其他平台 RSS）存 `before_map`、After 再读算差（`now > before` 才记，防负）→ `registry.record_once`；`Unload` 分支 → `registry.remove(id)`（OCR idle 释放时清条目，避免残留旧估算）。
...
- **手编 `models` 表 / `app_config` 表需重启进程生效**（`OnceLock` 缓存，运行中不可热更新；运行时修改走 `RuntimeConfig` + `persist_*`）。DB schema `user_version` 当前为 18（开发期简化：以 db.sql 为唯一真相，无历史迁移链；schema 变更直接改 db.sql + 升 user_version，删库重初始化；v17→v18 为首个增量迁移——FTS5 backfill）。
