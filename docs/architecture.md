...
- **Extension Package**（2026-07-10）：`~/.octopus/extensions/` 下含 `config.yaml`（YAML 元数据：name/description/version/author + action + rules 预留 + skill 预留）的文件夹 = 一个 Package。导入集成进菜单编辑（非独立子页）：新增子项选类型「扩展包」→ EditForm 拖拽区（zip/文件夹）或「选择文件/文件夹」→ `import_extension` 仅校验+重复检测（不复制）→ 保存时 `install_extension` 复制到 extensions + 创建 DB `action_bar_items` 记录（`action_data` 存脚本**绝对路径**，`action_type=script`）。`spawn_script` 检测 `action_data` 前缀：绝对路径→读文件内容 + 设 `OCTOPUS_PACKAGE_DIR` 环境变量；否则→内联。`config.yaml` 的 `skill` 块纯声明性（`ref` 关联 `~/.agents/skills` 或 `skill.file` 自带 SKILL.md），一期仅 config 预留 agent 接口（无独立 spec，设计以本节为准）。**⚠️ 已知限制——编辑死锁链（2026-07 审查发现，未修）**：编辑已安装的 extension 项时，DB `action_data` 是脚本绝对路径（不含 `|`），`saveEdit` 的 extension 分支 `actionData.split('|')` 得 `dirName=undefined` → 被「请先选择扩展」拦截，任何字段修改都保存不了；重选同一扩展包又因 `import_extension` 的 `dest.exists()` 检查被拒（"扩展包已存在"），形成死锁。另有：`install_extension` 签名缺 `shortcut`/`is_enabled` 参数（硬编码 `''`/`true`，EditForm 设置被静默丢弃）、extension 类型下 autoPaste toggle 隐藏（后端 script 分支仍读 `auto_paste`，Run And Paste 对扩展永久失效）、重装时新记录 `sort_order=MAX+1` 致菜单位置跳到同级末尾。**仅编辑已有 extension 项受影响，新建正常**。
