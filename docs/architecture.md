...
- **搜索驱动命令面板（2026-07-15，v32 迁移）**：ActionBar 从纯菜单条升级为搜索驱动的命令面板——用户可在搜索输入框中搜索应用、文件、菜单项、Quicklinks、书签。**新增独立 crate `octopus-search`**（matcher + app_index + file_search + bookmark + engine），不依赖 Tauri 可独立测试。**nucleo-matcher** 模糊匹配（exact > prefix > pinyin > fuzzy 四级优先级），拼音首字母硬编码常用菜单项。**搜索引擎** `SearchEngine` 持有应用索引 + 书签索引（启动时扫描），`search(query, tab)` 返回统一 `SearchResult`。tab 分两种搜索模式：`"quick"` 仅即时搜索（应用+菜单+Quicklinks，<16ms），`"files_bookmarks"` 延迟搜索（文件 mdfind + 书签，150ms 防抖）；`"all"` 合合两种。**DB v32**（v31→v32）：`action_bar_items` 加 `trigger_keyword TEXT`（Quicklink 关键词触发，如 "tr"）+ `auto_paste INTEGER`（Run And Paste 模式开关）。**前端**：ActionBar `index.tsx` 加搜索输入框（始终显示）+ 条件渲染（query 为空→菜单条，query 非空→SearchPanel Tab 栏+结果列表）；`SearchPanel.tsx`（5 Tab：`?全部` `a应用` `f文件` `>Shell` `b书签`）+ 展开方向判定（向下/向上）；纯逻辑提取为 `searchLogic.ts`（15 函数 + 55 单元测试）+ `searchTypes.ts`。键盘导航：输入框 Tab/↑↓→结果区、Enter→执行首项；结果区 ↑↓ 导航、Tab 循环 Tab 页、?/a/f/>/b 跳转、i 回输入框。**Quicklink 关键词触发**：输入 `tr hello` 匹配 trigger_keyword="tr" 的 URL 项 → 替换 URL 模板 `{query}` → 浏览器打开。**Run And Paste**：`auto_paste=true` 的 AI/脚本项执行后跳过 CompactEditor，直接写剪贴板 + 模拟 ⌘V 粘贴到光标（复用 `paste::paste`）。设置页加 trigger_keyword 输入框（URL 类型）+ autoPaste 开关（AI/Script 类型）。详见 [spec](superpowers/specs/2026-07-15-actionbar-search-design.md) + [plan](superpowers/plans/2026-07-15-actionbar-search.md)。
