...
**Cargo profiles 三层结构（2026-07-18）：** `[profile.release]` 空（默认 release，开发期快速链接）；`[profile.optimize]` 在 release 基础上叠 `strip = true` / `lto = "fat"` / `codegen-units = 1`（生产构建用，`cargo build --profile optimize`）；`[profile.profiling]` 在 release 基础上叠 `debug = "full"` + `split-debuginfo = "off"` + `strip = "none"`（perf 测量用，符号嵌进 binary 让 samply/atos 直接可读，详见 [research](superpowers/specs/research/2026-07-18-settings-mousemove-cpu-investigation.md) §6）。⚠️ `profiling` 当前 `inherits = "release"`（不含 LTO），如需测带 LTO 的真实性能临时改 `inherits = "optimize"`。
