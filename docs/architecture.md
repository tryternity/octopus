...
- **yaml 迁移**：全新库（`user_version < 17`）建库时检测旧 `~/.octopus/config.yaml` → 解析导入 DB 覆盖 seed → 重命名为 `config.yaml.bak`。迁移逻辑在 `init_schema` 中一次性执行（导入后库即 v35，后续启动跳过）。
...
- `models` 表无 `is_active` 列（开发期 schema 变更采用删库重初始化——见 `crates/infra/src/db.sql` 注释；`init_schema` 仅 `user_version < 31` 时执行 db.sql 建表+seed+yaml 导入（v35 跳过），v17+ 库跑增量迁移升到 v35，无 DROP 兜底）。
...
- **手编 `models` 表 / `app_config` 表需重启进程生效**（`OnceLock` 缓存，运行中不可热更新；运行时修改走 `RuntimeConfig` + `persist_*`）。DB schema `user_version` 当前为 35（开发期简化：以 db.sql 为唯一真相，增量迁移 v17→v35 链详见上方 octopus-infra 节；schema 变更直接改 db.sql + 升 user_version，删库重初始化）。
