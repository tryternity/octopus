...
- **结果窗光标定位 + 中间插入 + 选中替换**（[spec](superpowers/specs/2026-07-03-asr-cursor-insert-design.md) §1–§11）：
  ...
  - **选中替换**：拖选一段 → `invoke("set_selection", {start,end})` → `set_selection` 记录 `pending_delete` + 劈 `caret_gap` 到 start（**不立即删**，保留浏览器原生高亮）；首个非空 delta 到达时 `delete_range` 真删 + `push_delta_at_caret` 插入（删旧换新）。点别处 `set_caret` 清 `pending_delete` 取消。前端 `onMouseUp`（选区 mouseup 才完整）按 `isCollapsed` 分流（折叠→set_caret 中插；非折叠→set_selection）。
