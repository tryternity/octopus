...
- **Agent × 语音识别联动（2026-07-13，v27 迁移）**：agent 项含 `{{task}}` 时联动语音录音——用户选 agent 项后自动开始录音（复用现有 ASR 流程），识别结果作为 task 注入 agent 命令执行。**RecordType 枚举**（`coordinator.rs`）：`Input`（普通录音→paste）/ `AgentBridge { task_id }`（→ agent 执行），变体携带 task_id，match 穷尽。**Transcript** 加 `record_type` 字段贯穿录音全生命周期。`start_recording`/`begin_recording`/`prepare_streaming_session`/`prepare_vad_segmented_session`/`prepare_cloud_streaming_session` 全链路透传 record_type。**StartAgentRecording Command**（跳过 prepare-record 两阶段，agent 录音无 selection 需求）。**finalize_after_stop** 按 `record_type` match 分流——AgentBridge → `execute_agent_task`（独立函数：DB 取上下文 → 渲染 prompt → render_command → Terminal.app → 更新 task 状态 → 隐藏 Result）。**DB agent_tasks 表**（v27）：`id`/`status`(pending/executing/done/failed)/`agent_key`/`context`(JSON: kind/files/cwd/prompt_template)/`transcribed_text`(录音回调写入)/`error_msg`。**trigger_agent_voice 命令**：创建 task → 隐藏 action bar → `coordinator.start_agent_recording(task_id)`。`list_agent_tasks`/`delete_agent_task`/`retry_agent_task` 管理命令。**AgentPanel** 底部加任务列表区（状态色点 + 识别文本 + 重试/删除）。录音流程只携带轻量 task_id，不被 agent 逻辑污染；agent 上下文始终在 DB。详见 [spec](superpowers/specs/2026-07-13-action-bar-agent-voice-design.md) + [plan](superpowers/plans/2026-07-13-action-bar-agent-voice.md)。
