...
**translate_engine 配置语义**（2026-07-17 重构，Task 3-4）：从 spec 字符串（`"local:opus-mt"` / `"llm"` / `""`=自动）**改为 DB models 行的 id 字符串**——本地/云端统一表示。`TranslateStrategy` 解析（`action_bar_commands.rs::resolve_translate_strategy`）：`translate_engine` 解析为 `i64` id → `get_model_by_id` 查 DB 行 → 校验 `domain='translate' && is_enabled` → 按 `is_local` 分流 `LocalModel { row }` / `CloudModel { row }`。空 / 非数字 / 不存在 / 未启用 / 云端缺 secret_key → **`FallbackLlm` 分支**（复用润色 LLM `polish_llm` 兜底翻译，避免到 translate 时才报错；旧用户 `translate_engine` 仍是 spec 字符串时 `parse::<i64>` 自然失败走 fallback，不显式迁移）。引擎加载延迟到 `do_translate`（策略只决定路径不预加载）：opus-mt 按文本方向 `load_opus_mt(src, tgt)` 加载子目录、m2m100 按 `local:{model_name}` spec 加载、云端按 DB 行字段即时构造 `CloudLlmEngine::new`。
...
- `add_cloud_model` / `edit_cloud_model`（async）：LLM/Translate 保存是事务性操作——后端先调 `test_llm_connection`（POST `/chat/completions` + thinking disable），通过才写 DB；失败返回 Err 阻止保存。编辑时空 secret_key 从 DB 取真实 key 测试。`update` / `delete` WHERE 加 `AND is_local=0` 防御性守卫。**写 DB 后调 `reload_models_config()` 刷新推理缓存**（让 `RUNTIME_CONFIG` 看到新增的 is_enabled=1 云端模型，否则切换到它时推理路径找不到）。设置页列表不走缓存（`list_engines_from_db` 直查 DB），即使 reload 失败也即时反映。
...
**is_enabled 语义 = 文件就绪（v2）**：`true`=文件完备可被引擎加载，`false`=未就绪/未下载。写 DB 后调 `asr::config::reload_models_config()` 刷新 AsrConfig 缓存（`RUNTIME_CONFIG` v2 改 `RwLock<Option<Arc<AsrConfig>>>`，对齐 `APP_CONFIG` 模式），让「系统设置」引擎下拉即时更新——未就绪的模型不进下拉。local 模型 `secret_key` 重载为「文件清单 + sha256」JSON（api 模型仍是 key，按 `is_local` 分支，不冲突）。前端 React 组件 `frontend/src/pages/Settings/Models/`（AsrTab/LlmTab/OcrTab/TranslateTab + ModelRow + CloudModelForm，「模型管理」5 tab 布局，按 is_ready 显示「激活 / 下载 / 校验 / 删除 / 编辑」）。manifest（文件清单 + sha256，map 格式存 secret_key）下沉 `asr::manifest`，desktop/cli 共用；**cli `octopus-cli sync-models`** 批量扫描就绪本地模型、自举写 secret_key + 同步 is_enabled（首次填充/批量复核）。spec `superpowers/specs/2026-06-21-archived-spec.md#model-management-gui-design` §9。
...
- **DB 模型目录** `models` 表 → `asr::config::AsrConfig`（`octopus_asr_local::config::load_config()`，首次 `db::ensure_db()` 自动建表 + seed，读后缓存到 `RwLock<Option<Arc<AsrConfig>>>`）。**三层模型语义**：① `models.is_enabled=1` = **可用**（文件就绪/用户启用，可被选择，每域多个）；② `app_config.asr_engine` / `polish_llm` / `ocr_model` / `translate_engine` 指定**当前激活的那一个**（每域仅 1 个）；③ 推理时 `resolve_active_engine` 从可用集合（RUNTIME_CONFIG）里按 app_config 字段挑出激活引擎。**职责分离（2026-07-17）**：`RUNTIME_CONFIG` 仅缓存 ASR 域**可用模型**（`is_enabled=1`，供推理路径 `resolve_active_engine` / 各引擎 transcribe 挑选激活引擎）；`set_model_enabled`/`set_model_secret_key`/add/edit/remove_cloud_model 写 DB 后调 `reload_models_config()` 让推理缓存反映可用集变化。**管理列表（设置页 / 工具栏 / CLI）走 `list_engines_from_db()` 直查 DB**（`list_all_asr_engines`，全量含 is_enabled=0），不经 RUNTIME_CONFIG 缓存——新增/编辑/删除云端模型后即时反映，不依赖 reload（原 `list_engines` 走 load_config 缓存导致 reload 漏调时设置页不刷新的 bug 根除）。LLM/OCR/Translate 域无内存缓存，管理与使用均直查 DB。
...
- `models` 表无 `is_active` 列（开发期 schema 变更采用删库重初始化——见 `crates/infra/src/db.sql` 注释；`init_schema` 仅 `user_version < 31` 时执行 db.sql 建表+seed+yaml 导入（v36 跳过），v17+ 库跑增量迁移升到 v36，无 DROP 兜底）。
