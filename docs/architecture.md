...
- **yaml 迁移**：全新库（`user_version < 17`）建库时检测旧 `~/.octopus/config.yaml` → 解析导入 DB 覆盖 seed → 重命名为 `config.yaml.bak`。迁移逻辑在 `init_schema` 中一次性执行（导入后库即 v31，后续启动跳过）。
...
- `models` 表无 `is_active` 列（开发期 schema 变更采用删库重初始化——见 `crates/infra/src/db.sql` 注释；`init_schema` 仅 `user_version < 31` 时执行 db.sql 建表+seed+yaml 导入（v31 跳过），v17+ 库跑增量迁移升到 v31，无 DROP 兜底）。
...
- **手编 `models` 表 / `app_config` 表需重启进程生效**（`OnceLock` 缓存，运行中不可热更新；运行时修改走 `RuntimeConfig` + `persist_*`）。DB schema `user_version` 当前为 31（开发期简化：以 db.sql 为唯一真相，增量迁移仅 v17→v31 链——FTS5 backfill + ALTER TABLE 补列 + env seed + v22→v23 热词多版本 + v25→v26 agent 文件桥接 + v26→v27 agent_tasks + v27→v28 模型路径统一/manifest + v28→v29 env 去前缀 + v29→v30 OCR provider 修正 + v30→v31 云端模型自建；schema 变更直接改 db.sql + 升 user_version，删库重初始化）。
