...
- `models` 表：模型目录（**唯一来源**，schema v30，domain 支持 `'asr' | 'llm' | 'ocr' | 'translate'`）；`source`（`is_local=1`）= 路径标识（如 `asr/whisper-small`、`translate/opus-mt`、`ocr/PP-OCRv6-small`），相对于 `~/.octopus/models/`；`secret_key`（`is_local=1`）= **声明式下载清单 manifest JSON**（`{"<相对路径>": {"source", "sha256", "size}}`，source URL 支持 `{huggingface}` / `{github}` 模板变量，**不含 `env.` 前缀**），`secret_key`（`is_local=0`）= API Key；本地 ASR（is_local=1，13 行）+ 翻译（2 行）+ OCR（2 行，`provider='local', category='paddleocr'`）初始 manifest 由 `model_manifests.rs` 常量预填（从 HF cache 本地文件计算），首次下载后校验完整。唯一键 `UNIQUE(domain, provider, category, model_name)`
