...
独立的记事本业务 crate，仅依赖 `octopus-infra`。定位为「内容收集箱」——ASR / OCR / 转译记录的结果一键存为新笔记。**两类型**（`NoteType`：`text` 纯文本 / `markdown` md 源码，text 为默认）。文件 I/O 用 `std` + `dirs`。（富文本 TipTap/Html 已于 2026-07-02 彻底移除——历史 `type=html` 笔记由 v11→v12 迁移删除，`serialize` 模块与 `scraper` 依赖一并删除。）

| 模块 | 职责 |
|---|---|
| `model` | `NoteSource` 枚举（`asr`/`ocr`/`clipboard`/`manual`，serde 小写，manual 默认）+ `as_str`/`from_str`；`NoteType` 枚举（`text`/`markdown`，serde 小写，text 默认，`from_str` 未知值/历史 `html`→text 容错）+ `as_str`/`from_str`；`Note`（11 字段，含 `note_type`）；`NoteFilter`（含 `note_type` 按类型过滤） |
| `store` | CRUD + FTS5 检索 + 计数 + 排序（`is_pinned DESC, updated_at DESC, id DESC`）+ 分页 + `toggle_pinned`/`toggle_favorite`；`split_body(body, _type)` 简化——富文本已移除，`content_html` 恒空、`content_text`=body 原文（text）/md 源码（markdown）；CJK 三元分词：≥3 字符走 MATCH 短语，<3 字符 LIKE 回退 |
| `export` | 落盘 `~/Documents/octopus/notes/`（`dirs::document_dir` 跨平台）；`write_export`（stem 非法字符→`_` 防目录穿越 + 冲突 `-2/-3`）、`read_import`（读 .md/.txt 原文，前端按 type 载入） |

**DB**：`notes` 表（`crates/infra/src/db.sql`，`id INTEGER PRIMARY KEY AUTOINCREMENT`——与 clipboard/transcriptions 毫秒戳 id 不同，notes 有独立 `created_at`/`updated_at` 列；字段 title/content_text/content_html/type/source/source_ref_id/is_pinned/is_favorite/created_at/updated_at；`type TEXT NOT NULL DEFAULT 'text'` 为内容格式；`content_html` 列保留但不再使用）+ `notes_fts`（FTS5 `trigram` 分词，`content='notes'`）+ 3 触发器（insert/update/delete 同步 fts）。迁移经 `init_schema` 的 `PRAGMA user_version` v8→v9→v10→v11→**v12**（`crates/infra/src/db.rs`；v9→v10 幂等 `ALTER ADD type`；v10→v11 幂等 `ALTER ADD content_html` 兼容 egui 分支库；**v11→v12 `DELETE FROM notes WHERE type='html'`**——移除富文本，删历史 html 笔记，保留 text/markdown 不动）。
...
- `note_commands.rs`：12 个 Tauri 命令薄层 + 2 个集成入口（`save_transcription_to_note` / `save_ocr_to_note`，写入即 `emit("notepad://changed")`）。`create_note`/`update_note` 透传 `note_type`（前端按 type 传 body+type）；`save_transcription_to_note`/`save_ocr_to_note` 固定 `type=text`（直存原文）。（`save_clipboard_to_note` 已于 2026-07-01 移除——剪贴板条目不再存入记事本；`NoteSource::Clipboard` 变体保留供 DB 历史数据反序列化。`get_note_image`/`insert_note_image` 图片桥接命令已于 2026-07-02 随富文本移除删除。）
- `compact_editor_commands.rs` + `compact_editor_window.rs` + `compact_editor_native.rs`：**精简编辑器**（纯文本编辑工具，与完整版记事本并列——前者只编辑、结果经事件返回调用方、不落笔记；后者完整笔记：标题/分类/类型（text/markdown）/持久化）。**macOS 原生化（2026-07-03 webview→原生试水，三红线全过）**：macOS 用 `WindowBuilder`(tauri `unstable` feature) 建无 webview 原生窗 + `compact_editor_native.rs`（整文件 `#[cfg(macos)]`，objc2）挂 NSScrollView/NSTextView + 工具栏（NSView+NSButton 横排：撤销/重做/字号±/字号显示/查找/清空/字数/取消/保存）；非 macOS 回退 webview（`pages/CompactEditor/`，初始文本传递已随 PENDING 移除，fallback 暂不维护）。通信：open → `set_session`(requestId/SAVED 状态机) + `set_text`(直写 NSTextView)；保存/取消 → 后端 `do_save`/`do_cancel` `app.emit("compact-editor://result|cancel")` + 关窗兜底 cancel（`on_window_destroyed`），emit 全在后端 Rust（不受前端 ACL 管，记忆里「capability 漏列致保存不回传」坑天然消失）。快捷键由自定义 container NSView(`CompactEditorContainerView`) `performKeyEquivalent:` 拦（Cmd+Z/Shift+Z/F/Return/Esc，其余交 super 让 textview 原生处理 Cmd+C/V/A）+ 兼 textview delegate(`textDidChange:` 字数统计)；撤销/重做需 `setAllowsUndo(true)`、查找用系统 find bar(`performFindPanelAction:` tag=1)；字号持久化 `app_config`(`compact_editor.font_size`)。窗口单例 + 关窗即销毁，原生标题栏 720×560 可调（按 `contentLayoutRect` 布局避标题栏遮工具栏）；macOS 开窗切 Regular、关窗切回 Accessory（与 notepad/settings 对称）。详见 [试水 spec](superpowers/specs/2026-07-02-compact-editor-native-design.md) / [实施计划](superpowers/plans/2026-07-02-compact-editor-native.md) / [VALIDATION.md](../../VALIDATION.md)（per-window 内存 ~2M，首开 +32M 为 app 级一次性预热）。
...
- `clipboard_commands.rs` 新增 3 命令（图片预览专用）：`get_image_full`（取 `image_data.blob` 全分辨率 → `ipc::Response` 返回原始 WebP 字节，前端 `URL.createObjectURL` 加载）/ `save_image_dialog`（`ipc::Request` Raw body，前端传 `ArrayBuffer`）/ `copy_image_to_clipboard`（`ipc::Request` Raw body → `write_image` + `handle_clipboard_change` 全部在 `spawn_blocking`）。`copy_clipboard_item`（剪贴板条目复制）移入 `spawn_blocking` 避免 UI 冻结。
...
- 前端：`pages/Notepad/` 按 `note_type` 分发编辑器——text=textarea、markdown=`MarkdownEditor`（marked 编辑/预览切换）。新建笔记弹 type 选择菜单（纯文本/Markdown，已建锁定不可改）；`NoteList` 行显示 T/MD 角标 + 类型 tab（全部/纯文本/Markdown）+ 删除按钮；导入 .md/.txt 载入正文，导出按类型（text→txt、markdown→md）。`useNotes` hook（列表 + 搜索防抖 + 分页 + `notepad://changed` 监听）。富文本 TipTap 依赖（`@tiptap/*` + `tiptap-markdown`）已移除（前端 bundle ~1.2M→410K）。
...
详见 [type 迁移 spec](superpowers/specs/2026-07-02-notepad-type-migration-design.md)（含富文本移除说明）与 [实施计划](superpowers/plans/2026-07-02-notepad-type-migration.md)。（[2026-06-30 初版 spec](superpowers/specs/2026-06-30-notepad-design.md) 描述的是富文本移除前的 TipTap 设计，仅作演进留档。）
...
| `notepad_window` | 独立「记事本（内容收集箱）」窗口（单例 `get_webview_window`，1000×680，原生标题栏，仿 `settings_window`）。三栏布局：左侧列表（搜索 + 类型 tab：全部/纯文本/Markdown + 收藏过滤 + 分页 + 行内删除）、中间编辑器（text=textarea / markdown=`MarkdownEditor`，800ms 防抖自动保存 + md/txt 导入导出）、右侧空。命令：`open_notepad`（纯开窗）/ `open_notepad_with_note(noteId)`（存 `PENDING_NOTE` + 已开则 emit `notepad://select-note`、未开建窗；供 OCR 等场景「存笔记 + 打开并选中」）/ `get_pending_note`（前端 mount take）。托盘菜单「记事本」唤起；macOS 关窗 `Destroyed` 经 `on_notepad_closed` 回切 `Accessory` 激活策略（与 settings_window 对称）。业务逻辑见 [`octopus-notepad`](#octopus-notepad记事本--内容收集箱)。 |
| `compact_editor_window` | 精简文本编辑器窗口（720×560 可调、居中）。**与 `notepad_window` 并列但定位不同**——纯编辑工具，关窗即销毁，编辑结果经事件返回调用方，**不持久化为笔记**。**macOS 原生化（webview→NSTextView 试水）**：macOS 为无 webview 原生窗（NSWindow+NSScrollView+NSTextView + 工具栏 NSButton 横排），文本/IME/撤销/滚动全由 NSTextView 系统白送；保存/取消 emit 走后端 Rust（`do_save`/`do_cancel` + 关窗兜底 cancel）；快捷键 Cmd+Z/Shift+Z/F/Return/Esc + 字数统计由自定义 container NSView 拦 `performKeyEquivalent:` / `textDidChange:`。非 macOS 回退 webview（`pages/CompactEditor/`，初始文本传递已随 PENDING 移除）。单例：open 时已存在则 show+focus 并 `set_text` 换文本，否则建窗；macOS 开窗切 Regular、关窗 `Destroyed` 经 `on_compact_editor_closed` 切回 Accessory（与 notepad/settings 对称）。入口：OCR 识别后、剪贴板文本条目「编辑」按钮（语音结果窗**不用**独立编辑器——改为原地尺寸双模式，见 `result_window` 行）。详见 [试水 spec](superpowers/specs/2026-07-02-compact-editor-native-design.md) / [VALIDATION.md](../../VALIDATION.md)。 |
