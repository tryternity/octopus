...
8. **物理/逻辑坐标转换**（⚠️ 关键，反复踩坑 6+ 次）——macOS 有两套坐标 API，**必须区分**：
   ...
   - 曾误把 CGEvent 当物理坐标除 scale → 浮窗位置偏到完全无关的地方、副屏选中浮窗出现在主屏。`crates/desktop/src/action_bar_commands.rs::get_mouse_position`、`compact_editor_window.rs`、`window_position.rs` 均已修复。
