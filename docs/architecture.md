...
| `handle` | `ClipboardHandle`：`Mutex<ClipboardContext>` 全局单例（Windows 防锁竞争）+ `AtomicBool` suppress flag（区分 ASR 写入与外部复制，watcher 跳过自身写入）+ `AtomicBool` recording_enabled（`clipboard_enabled` 运行时镜像，false 时 `on_clipboard_change` 不入库；`set_config` 热重载翻转） |
