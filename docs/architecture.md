...
- **`switch_input_source_on_paste`**（默认 `true`，仅 macOS）：粘贴前临时切换到 ASCII 输入源（ABC）→ 模拟 Cmd+V → 恢复原输入源（三段式文本注入，参考 VoxFlow `VoxFlowTextInsertion`）。CJK 输入法 composing 状态下模拟 Cmd+V 可能乱码/丢字符，此配置根治。实现 `crates/desktop/src/input_source.rs`——Carbon TIS API（`TISCopyCurrentKeyboardInputSource` / `TISSelectInputSource`）via FFI，RAII guard `InputSourceGuard` 构造时切到 ABC（当前已是 ABC 则跳过，省 50ms）、drop 时恢复。`paste_via_clipboard`（ASR 粘贴）和 `focus_tracker::simulate_paste_platform`（剪贴板浮窗双击粘贴）两条路径均接入。**线程安全**：与 enigo `UCKeyTranslate` 的 SIGTRAP 不同，`TISCopyCurrentKeyboardInputSource` / `TISSelectInputSource` 是高层 API（getter + 通知投递），在 `spawn_blocking` 非主线程调用安全（select 的通知由主 RunLoop 异步处理，50ms 延迟足够）。
