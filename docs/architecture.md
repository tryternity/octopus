...
- `add_cloud_model` / `edit_cloud_model`（async）：LLM/Translate 保存是事务性操作——后端先调 `test_llm_connection`（POST `/chat/completions` + thinking disable），通过才写 DB；失败返回 Err 阻止保存。编辑时空 secret_key 从 DB 取真实 key 测试。`update` / `delete` WHERE 加 `AND is_local=0` 防御性守卫。**写 DB 后调 `reload_models_config()` 刷新 AsrConfig 缓存**（2026-07-17 修复：原先只有 `set_model_enabled`/`set_model_secret_key` 调 reload，add/edit/remove 漏调导致 ASR 云端模型新增后设置页列表不刷新、要等重启——因为 `list_engines` → `load_config` 经 `RUNTIME_CONFIG` 缓存，首次 load_models 填充仅 reload 时更新；llm/translate 域直接查 DB 无缓存不受影响）。
