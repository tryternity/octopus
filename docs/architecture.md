...
| `app_index` | 应用启动索引缓存（name/alias/path/icon，搜索器 launcher 用） | `AppIndex::scan` 扫盘写入，`reindex_apps` 刷新 |
