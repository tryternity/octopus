...
- **全局热键**：仅 `Cmd+Shift+L`（Auto-Type picker，默认 `vault_autotype_shortcut`）。**生成器为共享主体 + Modal 外壳**（`PasswordGenerator.tsx` + `PasswordGeneratorModal.tsx`，密码字段旁 🔑 按钮 → Modal），原 `Cmd+Shift+G` 浮窗 + `password_generator_window` 已废弃；未来 Actionbar 召唤独立窗口场景可复用主体（已预留 `onAutotype` prop）。`AppConfig.vault_generator_shortcut` 字段保留仅为兼容旧 DB。
