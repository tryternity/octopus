...
- **`switch_input_source_on_paste`**（默认 `true`，仅 macOS）：粘贴前临时切换到 ASCII 输入源（ABC）→ 模拟 Cmd+V → 恢复原输入源（三段式文本注入，参考 VoxFlow `VoxFlowTextInsertion`）。CJK 输入法 composing 状态下模拟 Cmd+V 可能乱码/丢字符，此配置根治。实现 `crates/desktop/src/input_source.rs`——Carbon TIS API（`TISCopyCurrentKeyboardInputSource` / `TISSelectInputSource`）via FFI，RAII guard `InputSourceGuard` 构造时切到 ABC（当前已是 ABC 则跳过，省 50ms）、drop 时恢复。`paste_via_clipboard`（ASR 粘贴）和 `focus_tracker::simulate_paste_platform`（剪贴板浮窗双击粘贴）两条路径均接入。**线程安全（⚠️ 已踩坑）**：TIS API 全家族**必须在主线程调用**——初版误认为线程安全，实机 SIGTRAP 崩溃（`Trace/BPT trap: 5`，与 enigo `UCKeyTranslate` 同源）。修复：所有 TIS 调用经 GCD `dispatch_sync_f` 调度到主线程执行（`_dispatch_main_q` 全局符号），粘贴路径在 `spawn_blocking` / `std::thread::spawn` 非 main 线程，不会死锁。详见 [spec §3](superpowers/specs/2026-07-10-input-source-switch-design.md#3-线程安全分析)。
