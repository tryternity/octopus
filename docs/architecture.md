...
- **DB 模型目录** `models` 表 → `asr::config::AsrConfig`（`octopus_asr_local::config::load_config()`，首次 `db::ensure_db()` 自动建表 + seed，读后缓存到 `RwLock<Option<Arc<AsrConfig>>>`）。**职责分离（2026-07-17）**：① `RUNTIME_CONFIG` 仅缓存 `is_enabled=1`（激活态），**仅供推理路径**（`resolve_active_engine` / 各引擎 transcribe）；`set_model_enabled`/`set_model_secret_key`/add/edit/remove_cloud_model 写 DB 后调 `reload_models_config()` 让推理缓存反映激活态变化。② **管理列表（设置页 / 工具栏 / CLI）走 `list_engines_from_db()` 直查 DB**（`list_all_asr_engines`，不过滤 is_enabled），不经 RUNTIME_CONFIG 缓存——新增/编辑/删除云端模型后即时反映，不依赖 reload（原 `list_engines` 走 load_config 缓存导致 reload 漏调时设置页不刷新的 bug 根除）。
