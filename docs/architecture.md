...
- `transcriptions` 表（schema v14，段模型）：识别历史，结构化真相源为 `segments`（段 JSON `[{kind:raw|polished|edited, text}]`）+ `text`（= `finish_text()` 扁平，供 search/clipboard/history 直读）；`raw_text`/`polished_text`/`edited_text` 三旧列降为兼容旧读路径（段模型下不再单独驱动展示）。另有润色状态（`polish_status`：`off`/`done`/`failed`）+ 元数据（engine / engine_mode / created_at / char_count / duration_ms）
- **过程增量入库（schema v14）**：`transcriptions.id` = 识别开始毫秒时间戳（`INTEGER PRIMARY KEY`，应用写入，去 `AUTOINCREMENT`），兼任主键 / 业务 key / 开始时间戳；`duration_ms = finalize_now_ms - id`。入库时机分散到识别过程各事件，每次同步写 `segments`（真相源）+ `text`（扁平）两列：首次有 ASR 文本 → `INSERT`（`insert_transcription_at_id`，写 segments + text）；分段 / 流式 partial → `update_text_segments`（同时写 raw_text + text + segments）；停顿润色完成 → `update_polished`（写 polished_text + segments + text）；停止 → `finalize`（含 `duration_ms`，`finalize_transcription`）。DB 失败仅 `warn` log 不阻塞识别（best-effort）。v13→v14 migration：ALTER 加 segments + text 列 + Rust 读旧三列（edited≻polished≻raw 优先级）映射为单段写回（纯 SQL 拼 JSON 无法转义换行/控制字符）。详见 [spec](superpowers/specs/2026-06-14-archived-design.md)
