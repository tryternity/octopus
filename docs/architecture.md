...
- **环境降噪（DeepFilterNet3，采集层前置）**：麦克风音频送入 VAD/ASR 前，经 `crates/asr/src/denoise.rs` 的 `DenoiseProcessor`（ONNX `dfn3.onnx`，48kHz STFT(sqrt-hann, n_fft=960, hop=480)→feat_erb/feat_spec→单帧推理→iSTFT+OLA）降低稳态/非稳态背景噪声。GRU 隐状态**跨 `drain_samples` 周期、跨 VAD 分段连续保持**（噪声估计是连续物理过程，与 `filter_vad` 每段 reset 故意相反）；新会话 `start()` 调 `reset()`。链路 `process_pipeline`：原生SR→(`down_sampler`)→48k→DenoiseProcessor→(`resampler`)→16k（`flush` 语义同重采样器：`drain_samples` 不 flush 保连续、`stop` flush 取尾）。由 `config.yaml.denoise_enabled`（默认 true）开关。**三级降级**：`denoise_enabled=false`→零开销直通；模型缺失 / `DenoiseProcessor::new` 失败→持 `None` + `warn`（含下载提示 `hf download penta2himajin/deepfilternet3-onnx`）→走原 16k 重采样直通；单帧推理失败 / 输出提取异常→该帧 bypass 原样本 + GRU 保持 + warn，**不 panic**（持锁路径无 panic 面，输出提取经 `try_extract_tensor` + 精确长度校验，防 Mutex poison 致录音功能挂死）。模型走 HF cache（`find_df3`），不进 DB、不参与引擎选择。`DenoiseProcessor` 经编译期断言 `Send+Sync`（固化持锁前提）。详见 [spec](superpowers/specs/2026-06-16-denoise-deepfilternet-design.md)
