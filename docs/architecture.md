...
- **命令查阅助手 + notify-rs 文件监听（2026-07-16，v36 迁移）**：新增「命令」Tab（⌥C）作为 CLI 命令查阅助手。**统一 launcher_index 表**（v36，合并 app_index + command_index，type 字段区分 "app"/"command"，旧 app_index 数据迁移后删表；load_app_index/save_app_index 改为 launcher wrapper 保持 search crate 透明）。**CommandIndex** 扫 PATH 收集可执行文件（brew/cargo/npm/系统命令统一覆盖，不硬编码目录），whats/apropos 批量建 map + brew desc 批量填英文描述（避免逐命令 spawn 卡死），读 launcher_index DB 缓存 keywords。**CommandProvider**（source="command"）：matches_tab="commands"（不进 all，独立 Tab），命令名+keywords+description fuzzy 匹配取最高分，回车复制命令名（action_type="copy"）。**后台 LLM 线程**：启动 60s 后为 keywords 为空的命令批量调 chat_text_with_prompt 生成中英文关键字（system prompt 要求"功能+同义词+中文翻译"），每轮 20 个 + 500ms 间隔防限流，生成一个存 DB + 刷内存。LLM 未配置时命令助手仍可用（英文描述 + 命令名匹配）。**notify-rs**（v8）：file_watcher 监听 /Applications 等 Create/Remove 事件，debounce 3s → refresh_app_index（秒级响应新装 app），保留 2 分钟数量轮询兜底（macOS FSEvents 对非用户文件可能漏事件）。新增依赖：notify（desktop crate）。详见 [spec](superpowers/specs/2026-07-16-command-assistant-design.md) + [plan](superpowers/plans/2026-07-16-command-assistant.md)。
