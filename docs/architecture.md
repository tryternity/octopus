...
- **结果窗光标定位 + 中间插入 + 选中替换**（[spec](superpowers/specs/2026-07-03-asr-cursor-insert-design.md) §1–§11）：
  ...
  - **前端单测基建（2026-07-04，commit e797e0f）**：引入 vitest 4 + jsdom 29；`measureCaretPx`/`codePointOffsetTo`/`codePointOffsetBefore` 抽到 `Result/caret.ts`（纯函数可测），`caret.test.ts`（9 测）锁 code-point → UTF-16 offset 对齐 + null/空容器分支；jsdom 无 `Range.getBoundingClientRect`，defineProperty 补零矩形；`renderResultNow` 组件级测试留后续。
