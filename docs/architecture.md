...
- `add_cloud_model` / `edit_cloud_model`（async）：LLM 保存是事务性操作——后端先调 `test_llm_connection`（POST `/chat/completions` + thinking disable），通过才写 DB；失败返回 Err 阻止保存。编辑时空 secret_key 从 DB 取真实 key 测试。`update` / `delete` WHERE 加 `AND is_local=0` 防御性守卫。
