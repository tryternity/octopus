...
- **结果窗光标定位 + 中间插入 + 选中替换**（[spec](superpowers/specs/2026-07-05-archived-design.md#三asr-光标定位与中间插入选中替换) §1–§11，已归档）：
  ...
  - **编辑态**：coordinator 主循环 `editing` 标志置位时 tick 跳过喂引擎（硬暂停）。`commit_edit(flat)` 整篇压成单 `Edited` 段（raw/polished 清零）+ UPDATE DB（v14 `segments` JSON + `text` 列）。**诊断日志（2026-07-19）**：硬暂停期间既不调 pipeline.tick（`[BE tick]` 阈值打点跑不到）也不产 `Speaking` 事件（绿条因前端 200ms 防抖转灰）——这是"编辑后绿条不亮"的预期行为。为让事后可复盘，`perf_log.rs` 扩展为承载**双重职责**：阈值性能日志（`[BE tick]`/`[FE writeDoc]`，根因定位后可移除）+ 状态机诊断日志（`[STATE]`/`[HEARTBEAT]`/`[SPEAKING]`/`[FE]`/`[WARN]`，长期保留）。`coordinator.rs::log_tick_heartbeat` 在三个 Tick 分支入口 1Hz 节流打 `[HEARTBEAT]` + 检测 editing 翻转即打 `[STATE]`（5 处精确触发点 enter/commit/toggle/cancel/discard 互补）。详见 [spec](superpowers/specs/2026-07-19-asr-edit-stall-observability.md)。
