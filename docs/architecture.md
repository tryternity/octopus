...
| `window_detect` | 区域截图窗口边界吸附：`WindowDetector` trait（跨平台抽象，仿 PinWindow）+ `pick_top_window` 命中纯函数（过滤自身 PID / layer<0 / 退化 bounds / 跨屏；按 layer 降序取最上层、同 layer 保留 z-order 靠前者 = 鼠标点处可见最顶窗口，z-order 遮挡 first-hit——被遮挡窗口永不命中；v1.1 owner/frontmost 过滤已移除，frontmost 判定会错判露出被遮挡窗口）+ macOS `CGWindowListCopyWindowInfo` impl（`MacOsDetector`，`monitor_containing` 用 `CGDisplay` 做跨屏判定）。v1 窗口级零额外权限（复用屏幕录制权限）；v2 预留 `Granularity::Element` AX 元素级（仅浏览器 bundle id）。 |
...
9. **AI 命令面板**（2026-07-08，菜单 DB 化 2026-07-09）——`action_bar_window` 迷你浮窗，全局热键触发。选中文本→模拟 Cmd+C→弹出两级菜单→AI/搜索/翻译/网页→CompactEditor 展示结果。菜单项存 `action_bar_items` DB 表（自引用 parent_id 两级菜单），5 种动作类型：submenu/ai/url/script/copy。script 按第一行 magic comment（`#shell`/`#osascript`/`#powershell`/`#python`/`#node`/`#deno`/`#bun`/`#javascript`/`#typescript`）分发运行时——后三种为自动探测（JS: node→bun→deno，TS: npx tsx→bun→deno，预探测 `--version`），选中文本通过环境变量 `$OCTOPUS_TEXT` 传递（不拼字符串，防注入）。script 类型支持 `is_async`（异步 fire-and-forget / 同步 spawn_blocking 等待）+ `write_output_to_clipboard`（仅同步+成功+有 stdout 时写剪贴板）。所有 script 执行结果落库 `script_runs` 表（stdout/stderr/exit_code/超时/耗时，截断 64KB），设置页「执行记录」子页可查看+清理。`run_script` 重构为 `spawn_script`（9 种 magic comment + 预探测）+ `wait_with_timeout`（try_wait 轮询 60 秒强杀 + 捕获 pipe）+ `run_script_async` / `run_script_sync_blocking`。图标支持文件名或内联 SVG（⚠️ 浮窗和设置页已改数字徽章，`icon` 字段保留兼容但不再 UI 展示）。设置页「命令面板」tab 做 CRUD + 排序（递归 TreeNode 树形控件：chevron 展开/收起 + 注册表式等宽序号 `01`/`1.1` + 全部展开/收缩按钮；新增用内存草稿模式——不写 DB，保存时才 create，取消零脏数据。详见 spec §6.3）。**键盘导航（⚠️ 强需求）**：上下键切焦点层（main↔sub），左右键当前行移动（主菜单 submenu 项自动展开子菜单预览但不抢焦点，非 submenu 项收起），**1-9 + a-z 快捷定位第 1-35 项**（只移动高亮不执行，超出范围无效；同级上限 35 个，后端 create 校验），菜单溢出时 scrollIntoView 自动滚动 + voice 色 <</> 箭头指示器（ScrollRow 组件），定位到 submenu 项同步展开预览，执行用 Enter），Esc 直接关闭浮窗（一次）。`focusLayer` 状态独立于 `view` 状态。**窗口焦点策略（⚠️ 强需求）**：全局快捷键不得将 settings/compact_editor 带到前台。macOS 上 WKWebView 要求 app 进程 active 才能获得键盘焦点，而 `set_focus` 触发 `NSApp.activate` 会把所有可见 Regular 窗口带到前台。解法（视觉焦点协调方案）：(1) show 前记录前台 app（`NSWorkspace.frontmostApplication`）+ app 非活跃时临时隐藏其他可见窗口（`WINDOWS_TO_HIDE_ON_FLOAT`：settings/compact_editor/clipboard_window）；(2) `set_focus` 激活浮窗获得键盘焦点（此时 Regular 窗口已隐藏不被带出）；(3) hide 时先 `activate` 原前台 app 交还焦点，再 `show` 恢复被隐藏的 Regular 窗口（此时 app 在后台，窗口温和恢复）。实现：`activation::before_floating_window_show` / `after_floating_window_hide` 公共函数（`FLOAT_DEPTH` 引用计数支持多浮窗嵌套——只有最外层 depth==1 记录状态/交还焦点），action bar + 剪贴板浮窗共用。`action_bar_show_result` 调 `after_floating_window_hide_keep_active`（递减 depth + 清状态 + 恢复隐藏窗口，跳过 deactivate 避免 CompactEditor 被压后台）。剪贴板浮窗另加 `Focused(false)` 事件回调 `restore_hidden_windows_only`（失焦 = 虚拟关闭：`float_depth_decrement_and_is_zero` 扣减——**depth>0（仍有浮窗存活如 action_bar）时直接 return 不清状态**，只有 depth==0 才清 `WAS_INACTIVE`/`PREV_APP` + 恢复隐藏窗口 + deactivate。纯逻辑提取为 `float_depth_increment` / `float_depth_decrement_and_is_zero` / `float_clear_state`，单测覆盖 5 场景）。语音识别窗无强键盘操作需求，保持现状。`TRIGGER_IN_PROGRESS: AtomicBool` 重入 guard。`execute_action_bar`（async command）收口重构为 inner fn（返回 `Result<bool>`）+ 三路 match：Ok(true)=ai 已自收口直通、Ok(false)=url/script/copy 成功 hide+finalize、Err=异常仅 finalize（不 hide，前端显示红色气泡提示）——确保 `?`/Err 不会泄漏重入锁和 depth。**线程约束**：Ok(false) 的 `hide_action_bar_window` 经 `run_on_main_thread` 投递主线程（async command 跑 worker 线程，`after_floating_window_hide` 的 `NSApplication::deactivate` 需 `MainThreadMarker`）；`finalize_action_bar` 仅 AtomicBool 线程安全即时执行。前端不再直接 `getCurrentWindow().hide()`，url/script/copy 也走 try-catch，所有错误统一红色气泡（半透明覆盖浮窗顶部，2 秒消失，error 视图已移除）。trigger 阶段 suppress watcher → Cmd+C → 读选中 → 立即恢复剪贴板（选中文本不入库）；**所有路径（含成功路径）显式 `clear_suppress`** 撤销——剪贴板未变化时 watcher 不触发，若不清除残留 flag 会吞掉用户下次手动复制。浮窗每次唤起都重新加载菜单+配置（非仅 mount 时）。统一执行入口 `execute_action_bar(item_id, text, app)` 按 action_type 分发，URL 参数编码复用 `percent-encoding` 库（`url_encode_param`）。仅 macOS 支持。capabilities 白名单必须包含 `action_bar_window`。
