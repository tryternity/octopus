...
| `denoise` | DeepFilterNet3 流式环境降噪（`dfn3.onnx`，48kHz STFT(Vorbis)→feat_erb(dB+EMA归一化)/feat_spec(单位归一化)→conv_lookahead=2 帧对齐→ONNX→iSTFT+OLA，GRU+归一化EMA状态跨帧保持），采集层前置 |
...
- **环境降噪（DeepFilterNet3，采集层前置）**：麦克风音频送入 VAD/ASR 前，经 `crates/asr/src/denoise.rs` 的 `DenoiseProcessor`（ONNX `dfn3.onnx`，48kHz STFT(Vorbis 窗, n_fft=960, hop=480)→feat_erb(band互相关功率→dB→EMA均值归一化/40) + feat_spec(前96bin→单位归一化)→conv_lookahead=2帧对齐(spec[t]配feat[t+2])→单帧推理→iSTFT+OLA）降低稳态/非稳态背景噪声。GRU 隐状态 + 归一化 EMA 状态 **跨 `drain_samples` 周期、跨 VAD 分段连续保持**（噪声估计是连续物理过程，与 `filter_vad` 每段 reset 故意相反）；新会话 `start()` 调 `reset()`。链路 `process_pipeline`：原生SR→(`down_sampler`)→48k→DenoiseProcessor→(`resampler`)→16k（`flush` 语义同重采样器：`drain_samples` 不 flush 保连续、`stop` flush 取尾）。由 `config.yaml.denoise_enabled`（默认 true）开关。**三级降级**：`denoise_enabled=false`→零开销直通；模型缺失 / `DenoiseProcessor::new` 失败→持 `None` + `warn`（含下载提示 `hf download penta2himajin/deepfilternet3-onnx`）→走原 16k 重采样直通；单帧推理失败 / 输出提取异常→该帧 bypass 静音 + GRU 保持 + warn，**不 panic**。模型走 HF cache（`find_df3`），不进 DB、不参与引擎选择。详见 [spec](superpowers/specs/2026-06-16-denoise-deepfilternet-design.md)
...
- **音频处理**: cpal（录音）、rubato（重采样，含 denoise 48k 桥接）、rustfft（STFT/iSTFT，denoise）、hound（WAV 读取）
- **Web 框架**: Axum + Tokio
- **桌面框架**: Tauri 2
