...
| `firered` | FireRedASR2-AED CTC 离线识别（小红书 FireRedTeam；单 ONNX `model.int8.onnx`(740M) + `tokens.txt`(vocab=8667)；encoder+CTC branch 导出、弃 attention decoder；80-bin fbank 复用 `sensevoice::compute_fbank`（无 LFR）+ CMVN 从 ONNX metadata `cmvn_mean`/`cmvn_inv_stddev` 读、公式 `(fbank-mean)*inv_std`；greedy CTC blank=0；中文+20方言+英） |
