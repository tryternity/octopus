...
- **应用上下文获取（app_context 模块，2026-07-12）**：trigger 时获取选中文本的来源应用与前后文，让 AI 动作具备情境感知。**异步架构**：浮窗先弹（仅 text）→ 后台线程 gather → 完成后回填 PENDING_CONTEXT（校验 `ctx.text == text_for_gather` 防跨触发污染）。`ContextProvider` trait + cfg 分发，macOS **双路径**：① **AX**（原生 App）：NSWorkspace 取前台 App → AXUIElement 读焦点元素 → `AXValue` + `AXSelectedTextRange` 切前后文，deadline 透传到 `find_text_element_depth` 每层检查；② **Browser AppleScript JS**（Chrome/Edge/Safari）：`execute javascript` 读真实 DOM，TreeWalker 定位 → 向上溯源 parentNode → indexOf 切前后文。JS 写唯一临时文件（纳秒时间戳）+ `read POSIX file`，RAII guard 确保删除。osascript 用 `spawn` + `Stdio::piped()` + `try_wait` 轮询超时 + `kill`（与 `wait_with_timeout_secs` 范式一致）。Firefox 无 AppleScript JS 接口 → fallback AX。Windows/Linux 留 NullProvider stub。Terminal 特例：AXSelectedTextRange 与 AXValue 不在同一坐标系（iTerm2），改用 selected_text 在 full_text 中搜索定位。自绘编辑器（Sublime/WPS）AX 树无真实内容 → 内容校验降级返回 None。上下文日志写入 `~/.octopus/logs/action-bar.log` 含 AX 诊断。before/after 各 1000 字截断。**CF 类型安全**（踩坑）：所有 AX 返回的 CFTypeRef 用前必须 CFTypeID 检查（`is_cf_string`/`is_cf_array`/`is_cf_value`），子元素 CFRetain 防 use-after-free。AX 属性名用 `CFString::new("AXFocusedUIElement")` 构造（extern static 不可链接）。**失败降级铁律**：`gather()` 任何失败只记日志、`ActionBarContext` 仅含 `text`，绝不阻塞浮窗。LLM 动作经 `build_enriched_text` 注入上下文；本地翻译不注入。41 个单测。详见 [spec](superpowers/specs/2026-07-12-actionbar-app-context-design.md) + [plan](superpowers/plans/2026-07-12-actionbar-app-context.md)。
