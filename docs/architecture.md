...
**跨平台贴图窗口（pin_window）：** 截图后「钉住」功能——创建原生浮动窗口置顶显示截图，支持拖拽（左键）、缩放（滚轮，以鼠标位置为锚点）、关闭（右键菜单）。绕过 WebView 直接用原生窗口，单窗内存 < 5MB。三平台各自实现 `PinWindow` trait（`create(png_data, x, y, w, h)`）：**macOS** 自定义 `PinNSWindow`（`define_class!`）+ `PinNSImageView`（拖拽经 `performWindowDragWithEvent`、缩放改 frame、右键 NSMenu 关闭）；静态 `PIN_WINDOWS: Mutex<Vec<SendWindow>>` 跟踪窗口 + `setReleasedWhenClosed(false)` 防悬空 + 主线程延迟 `cleanup`。**Windows** Win32 `WS_EX_TOPMOST|LAYERED|TOOLWINDOW` + `UpdateLayeredWindow`（预乘 BGRA + GDI `StretchBlt` HALFTONE 缩放）；每窗独立线程跑 `GetMessageW` 循环（显式判 0/-1）；GDI 资源经 `run_gdi_calls` 闭包 + defer 清理防泄漏。**Linux** GTK3 Toplevel + Cairo 自绘（`scroll-event` 缩放锚定 `event.coords()` + `win.move_()`）。`screenshot_commands::pin_screenshot` 双路径：前端 `composeAndCropBytes` 合成带标注/马赛克的 Canvas PNG → `FileReader.readAsDataURL` 转 base64 → 后端解码（`img_base64: Option<String>`）；None 时 fallback 到后端 `ALL_CAPTURES` 裁剪（不含标注）。前端 `isPinningRef` 防重复点击锁。详见 [spec](docs/superpowers/specs/2026-07-06-cross-platform-pin-window-design.md)。
