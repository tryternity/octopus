...
**入口**：托盘菜单「记事本」；剪贴板浮窗条目 + Settings 剪贴板管理页「存入记事本」；Settings 历史记录管理页「存入记事本」。（语音结果窗已支持原地大窗口编辑，「存入记事本」按钮已移除——长篇模式直接在结果窗编辑，无需导入记事本；后端 `save_transcription_to_note`/`current_transcription_id` 命令保留作基础设施。）OCR 命令已注册，MVP 不在 OCR 自动流程强制加按钮。
