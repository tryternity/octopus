...
| `sensevoice_orig` | 原版 SenseVoice-Small 离线识别（FunASR 4 输入 ONNX：`speech`[560维 LFR m=7/n=6]+`speech_lengths`+`language`(0=auto)+`textnorm`(1=无itn)；**CMVN 必须外部应用**——读 `am.mvn`（kaldi `<AddShift>`+`<Rescale>` 各 560 维）在 LFR 后做 `(feat+addshift)*rescale`，FunASR `WavFrontend` 标准 fbank→LFR→CMVN 不进 ONNX 图、`config.yaml` 的 `cmvn_file:null` 是训练残留字段；vocab=25055、blank=0；tokens.json 为 string 数组；中/英/粤/日/韩。**`skip_corrector=true`**：高质量模型跳过通用 corrector（与 qwen3 同机制，`pipeline.rs` 消费）——corrector 的 n-gram 同音纠错对高质量模型过纠有害，曾把模型正确输出的"开始语音识别"误纠成"开始于饮食别"（"始语→始于""音识→饮食"）。诊断教训：直接调 `engine.transcribe()` 的 e2e 测试绕过 pipeline 的 corrector，会掩盖此类问题，须走完整 pipeline 或断言原文。**sherpa nano 简化版（旧 category=`sensevoice`）已移除**，见 [docs/removed-sensevoice-sherpa-nano.md](removed-sensevoice-sherpa-nano.md) |
...
| `firered` | FireRedASR2-AED CTC 离线识别（小红书 FireRedTeam；单 ONNX `model.int8.onnx`(740M) + `tokens.txt`(vocab=8667)；encoder+CTC branch 导出、弃 attention decoder；80-bin fbank 复用 `fbank::compute_fbank`（无 LFR）+ CMVN 从 ONNX metadata `cmvn_mean`/`cmvn_inv_stddev` 读、公式 `(fbank-mean)*inv_std`；greedy CTC blank=0；中文+20方言+英） |
...
| `store` | DB CRUD：`insert_clipboard_item` / `insert_asr_item`（source=asr，关联 transcription_id）/ `insert_ocr_item`（source=ocr，engine/model 复用 clipboard_history 列存 OcrMeta）/ `query_history`（FTS5 `clipboard_history_fts` JOIN 搜索：查询 ≥3 字符包成 phrase 走 trigram 索引 MATCH，<3 字符 LIKE 子串 fallback；+ 6 种过滤 + 分页 + `ORDER BY created_at DESC, id DESC` 二级排序消除秒级 `iso_now` 同秒不稳）/ `get_item_by_id`（按 id O(1) rowid 读单条，供复制/粘贴/保存/打开/OCR/缩略图等单条命令；替代旧的 `query_history` 整页 + `.find` 线性扫描）/ `count_history`（与 `query_history` 同一套 `build_where` + LIKE-fallback/FTS5-MATCH 的 COUNT，供 `clipboard_stats(filter, search)` 按当前筛选/搜索返回准确条数，替代旧的无视筛选的 `count_all`）/ `toggle_favorite` / `delete_item`（+ 删除计数器 `track_deletes` + image_data 引用计数回收）/ `delete_items`（批量删除：单 SQL `DELETE ... IN (...)` + 一次 `track_deletes(总数)` 最多触发一次 FTS rebuild + 一次 `cleanup_unreferenced_images`，供设置页批量删除，替代前端循环单条）/ `delete_by_transcription_ids`（级联删除：Settings 删转译记录时同步删剪贴板引用，+ `track_deletes` 计入 FTS rebuild 阈值，与 `delete_items`/`clear_history` 一致；ASR 条目无 blob_hash 故不调 cleanup）/ `clear_history`（保留收藏）/ `rebuild_fts_index`（FTS5 索引重建，启动时 + 删除计数达 10 自动调用）/ `insert_image_data` / `get_image_blob` / `get_image_thumb` / `cleanup_unreferenced_images` + 去重（`find_by_content_hash` 图片 / `find_by_text(text, ItemType)` 文本+文件按类型匹配；旧实现 `find_by_text` 硬编码 `item_type='text'` 致文件去重失效，已修） |
...
**ASR 集成：** `coordinator.rs::do_paste` 中先调 `store::insert_asr_item`（写 DB source=asr，关联 `transcription_id`），**成功后主动 `emit("clipboard://changed")`**（`paste::paste` 写剪贴板设 suppress flag，watcher 的 `on_clipboard_change` 命中后直接 return、不执行含 emit 的 `on_change` 闭包，故 ASR 记录需主动广播前端才能即时渲染，详见剪贴板 spec §3.3），再调 `paste::paste`（写剪贴板，suppress flag 阻止 watcher 重复记录）。**级联删除（单向）：** Settings 删除转译记录时，`delete_history` 同步调 `delete_by_transcription_ids` 清理剪贴板中引用该记录的条目（DB `PRAGMA foreign_keys` 关闭，正向级联靠**应用层硬删**而非 schema 的 `ON DELETE SET NULL`——后者在外键关闭下是 no-op；若开外键会先 SET NULL 把 `transcription_id` 置空，导致硬删按 id 找不到而失效），**删除行数 >0 时主动 `emit("clipboard://changed")`** 广播浮窗/设置页双端刷新（否则浮窗 stale 显示已删条目、双击粘贴查不到 id 失效）；反向不级联（剪贴板删除语音条目只删 `clipboard_history` 行，外键 `ON DELETE SET NULL` 处理引用置空，`transcriptions` 源数据不受影响）。曾发现旧数据 `transcription_id` 为 NULL 导致级联失效（旧二进制未关联），已清理并加测试断言验证。

**DB 表：** `clipboard_history`（全字段：item_type/source/content/search_text/is_favorite/created_at + image 元数据 blob_hash/width/height/has_thumbnail + file_count + is_rich + ASR 元数据 transcription_id/polish_status/engine/model）+ `clipboard_history_fts`（FTS5 虚表，trigram tokenizer）+ 3 触发器自动同步（`clip_fts_au` 收窄为 `AFTER UPDATE OF search_text`——`touch_created_at`/`toggle_favorite` 等非搜索字段更新不再无谓重建 FTS 索引项；v7→v8 迁移 DROP+CREATE 使旧库生效）+ `image_data`（图片 BLOB 存储：hash/blob/thumb/image_type/width/height/created_at）。**FTS5 索引维护**：FTS5 external content table 的 DELETE 触发器只移除逻辑索引，`_data` 表 b-tree 页不收缩，删除越多空洞越大。维护策略——启动时 rebuild 一次（`main.rs` setup）+ 运行中删除计数器（`AtomicU32`，阈值 10）达 10 自动 rebuild + 清零（批量 `delete_items` 一次性累加，删 N 条最多 rebuild 一次）。**图片存储**：剪贴板图片以 WebP 无损 BLOB 存 `image_data` 表（替代旧文件系统 `~/.octopus/clipboard_images/`），`clipboard_history.blob_hash` 引用 `image_data.hash`；删除条目时引用计数为 0 才删 image_data 行（应用层实现，非 DB 外键）。启动时 `image_migration` 模块自动迁移旧文件到 DB。
...
记事本 crate 已于 2026-07-03 彻底移除——剪贴板历史 + 多 tab 精简编辑器已覆盖编辑/查看需求，独立的标题/分类/持久化笔记不再维护。`crates/notepad/` 删除；desktop 的 `notepad_window.rs` / `note_commands.rs` 删除；DB v12→v13 迁移 DROP `notes` 表 + `notes_fts` + 3 触发器；前端 `pages/Notepad/` 删除；托盘「记事本」菜单 + Settings「存入记事本」入口移除。OCR / 文本编辑统一改走 CompactEditor 多 tab（见 [`compact_editor_window`](#compact_editor_window-1)）。详见 [清理 spec](superpowers/specs/2026-07-03-clean-used-feature-design.md) 与 [实施计划](superpowers/plans/2026-07-03-clean-used-feature.md)。

**桌面端集成**（`octopus-desktop`）：
- `compact_editor_commands.rs` + `compact_editor_window.rs`：**精简编辑器（多 tab）**——每个 tab 绑定一个 clipboard item_id，可同时编辑/查看多个剪贴板条目。窗口单例 + 关窗即销毁，原生标题栏 720×560 可调；4 个命令——`open_compact_editor_tab(itemId)`（写 `PENDING_TAB` + 已开则 emit `compact-editor://open-tab` 推送新 itemId 并聚焦、未开建窗）/ `get_pending_compact_tab`（前端 mount take 首个 itemId）/ `get_clipboard_item_text(itemId)`（读条目 content 供 tab 载入）/ `close_compact_editor`（关窗）。前端 tab 标题 = 文本前 5 字 + `-` + id hex 后 5 位；Ctrl+S / Cmd+↵ 经 `set_clipboard_item_text` 回写 DB + 系统剪贴板；关 tab 不删条目（仅关视图）。macOS 开窗切 Regular、关窗切回 Accessory。
- `image_preview_commands.rs` + `image_preview_window.rs`：**图片预览**（剪贴板图片项的轻工具栏预览窗）。窗口 880×620（min 400×320，原生标题栏、可调、居中），按需创建；3 命令——`open_image_preview` / `get_pending_image` / `close_image_preview`。前端浮动白卡工具栏 + 灯箱暗场棋盘格画布 + 自然像素坐标标注（选择/矩形/椭圆/菱形/直线/箭头/画笔/文字/序号/马赛克 + 撤销/重做 + 实心填充 + 属性浮窗（操作时自动收起））+ 动作（保存/复制/OCR/缩放/置顶）。OCR = `ocr_image`（纯识别）→ `insert_ocr_clipboard_item` → `openCompactEditorTab`（新建 source=ocr 条目并在精简编辑器打开绑定 tab）。共享标注核心 `frontend/src/lib/annotation.ts`。**性能优化（2026-07-03）**：① 视口渲染——canvas 固定窗口大小（~2M 像素），滚动时只从预缩放位图裁剪可见区域画到 canvas（不管图多大，GPU 合成恒定 ~8MB）；② 底图 canvas + SVG overlay——标注用 SVG 元素（`AnnotationSvg.tsx`，标注变化零 canvas 操作）；③ zoom 走 `createImageBitmap` 异步预缩放（GPU 加速、`zoomVersionRef` 防过时帧）；④ 先 thumb 再 full 渐进加载（`cancelled` 防竞态，fit-to-width 默认 + `fitModeRef` 三态 + ResizeObserver 窗口 resize 自动重算）；⑤ thumb→full 期间 `loadingFullRef` 门控禁止标注；⑥ blob URL 泄漏修复 + EXIF 条用 `fullNatW/H`。详见 [perf spec](superpowers/specs/2026-07-03-image-viewer-perf-design.md)。
...
- 前端：`pages/CompactEditor/`（多 tab：tab 栏 + 工具栏撤销/重做/字号/查找替换/清空/保存 + textarea，**键盘/按钮 undo/redo 统一走 `document.execCommand`**——受控 textarea 每次 value 同步清空 WebKit 原生 undo 栈致键盘 Cmd+Z 失灵，详见 [清理 spec §12](superpowers/specs/2026-07-03-clean-used-feature-design.md)）；`lib/compactEditor.ts::openCompactEditorTab(itemId)`。`pages/Notepad/` 已删除。
...
| `notepad_window` | （已移除 2026-07-03）记事本窗口随 `octopus-notepad` crate 一并删除。 |
| `compact_editor_window` | 精简文本编辑器窗口（原生标题栏、720×560 可调、居中）。**多 tab**——每个 tab 绑定一个 clipboard item_id，可同时编辑/查看多个剪贴板条目；tab 标题 = 文本前 5 字 + `-` + id hex 后 5 位。工具栏（撤销/重做/字号/查找替换/清空/保存）+ textarea，关窗即销毁。Ctrl+S / Cmd+↵ 经 `set_clipboard_item_text` 回写 DB + 系统剪贴板；**清空后保存 = 删除条目**（空内容无意义，调 `delete_clipboard_item`：仅剩该 tab 则关窗，否则关该 tab）；关 tab 不删条目（仅关视图）。命令：`open_compact_editor_tab(itemId)`（写 `PENDING_TAB` + 已开则 emit `compact-editor://open-tab` 推送新 itemId 并聚焦、未开建窗）/ `get_pending_compact_tab`（前端 mount take 首个 itemId）/ `get_clipboard_item_text(itemId)`（读 content 供 tab 载入）/ `close_compact_editor`。单例：open 时已存在则 show+focus，否则创建；macOS 开窗切 Regular、关窗 `Destroyed` 经 `on_compact_editor_closed` 切回 Accessory（与 settings 对称）。入口：OCR 识别后（统一 insert_ocr_clipboard_item → openCompactEditorTab）、剪贴板文本条目「编辑」按钮（语音结果窗**不用**独立编辑器——改为原地尺寸双模式，见 `result_window` 行）。 |
| `image_preview_window` | 图片预览窗口（原生标题栏、880×620 可调、居中、min 400×320）。剪贴板图片项的轻工具栏预览——**frontend-design 浮动白卡工具栏 + 灯箱暗场棋盘格画布 + 底部 EXIF 条**；画布显示全分辨率图（默认 fit-to-width、zoom 0.1–8×、超窗滚动 + 抓手平移 + 自适应窗口/宽度切换）+ 标注（选择/矩形/椭圆/菱形/直线/箭头/画笔/文字/序号/马赛克 + 撤销/重做 + 实心填充 + 属性浮窗）+ 动作（保存/复制/OCR/缩放/置顶）。标注用自然像素坐标（缩放不错位）；复制=写系统剪贴板+主动入库历史；OCR=`ocr_image`（纯识别）→`insert_ocr_clipboard_item`→`openCompactEditorTab`（新建 source=ocr 条目并在精简编辑器打开绑定 tab）。单例：open 时已存在则 show+focus 并 emit `image-preview://load` 推送新 imageId，否则创建；macOS 开窗切 Regular、关窗 `Destroyed` 经 `on_image_preview_closed` 切回 Accessory（与 compact_editor 对称）。入口：剪贴板浮窗图片条目「预览」按钮（eye-edit 图标，`open_image_preview({imageId})`）；与双击粘贴互不冲突。共享标注核心 `frontend/src/lib/annotation.ts`。 |
| `image_migration` | 一次性迁移：`~/.octopus/clipboard_images/` → DB `image_data` BLOB。幂等（已存在的 hash 跳过），迁移成功后删除目录。启动时 `main.rs` setup 阶段调用。 |
| `clipboard_window` | 剪贴板历史浮窗（300×600，无边框圆角透明置顶，Alt+V 唤起——toggle 按焦点判断：失焦状态按快捷键直接 `show`+`set_focus` 激活，仅「可见且有焦点」才收起，避免 always-on-top 窗口失焦后仍 visible 导致需按两次；窗口位置记忆）。顶部标题栏（X + 「剪贴板」 + 右侧两 toggle 成组：监听开关 CircleCheck（绿圆+勾=监听中）/ CircleX（红圆+叉=已关闭）（`clipboard_enabled` 热重载，复制敏感内容前可快速暂停记录）+ Pin，`data-tauri-drag-region="deep"` + `cursor-grab`，点标题文本/空白均可拖动窗口，按钮因 clickable 元素被 drag.js 跳过、仍正常可点），搜索框 + 6 类过滤（全部/语音/文本/图片/文件/收藏，纯图标 tooltip），列表（hairline 分隔线，ASR 条目左侧 voice 色条，图片条目内联 WebP 缩略图，hover 显示操作按钮（编辑/预览/保存/OCR/打开/删除 + **收藏置末**——已收藏 amber 常显高亮，置首会遮其后 hover 按钮）（图片项「预览」Maximize2 → `open_image_preview` 开预览窗，与双击粘贴互不冲突）；**操作栏容器 `onDoubleClick` 阻止冒泡**——连续快速点操作按钮（如删多条）的 dblclick 不冒泡到条目，不被误判为双击粘贴+隐藏浮窗（click 层 `stopPropagation` 拦不住 dblclick 这个独立事件；与左侧类型图标 dblclick 拦截对称）；**左侧类型图标单击即复制**（`copy_clipboard_item`，触效：icon 放大回弹 + 闪绿 + 「已复制」气泡 1.5s；`stopPropagation` 不触发选中、双击拦截不误触粘贴））。单击选中不关闭（点类型图标则复制、不选中），双击写剪贴板 → 隐藏浮窗 → 模拟 Cmd+V 自动粘贴（`paste_clipboard_item`，后端串起 hide `clipboard_window` + `focus_tracker.restore_focus` + `simulate_paste`）。复制/粘贴按条目类型还原真实内容——文本 `write_text`、图片从 `image_data` 读 WebP 原图转 PNG 后 `write_image`、文件解析 JSON 路径后 `write_files`（`write_item_to_clipboard` 统一分发；旧实现无视类型一律 `write_text`，导致图片粘出 blob_hash、文件粘出 JSON 字符串）。图片条目可保存到 `~/Downloads/octopus/`（自定义浮层选格式 JPEG/WebP/PNG + 质量，默认 JPEG 85%，可选保存后打开文件夹）。OCR（`ocr_image` 纯识别）→ `insert_ocr_clipboard_item` 新建 source=ocr 条目 → `openCompactEditorTab` 在精简编辑器打开绑定 tab 编辑（Ctrl+S 经 `set_clipboard_item_text` 回写）。剪贴板过滤新增 OCR 类别（全部/语音/文本/OCR/图片/文件/收藏），source=ocr 条目用 ScanText 图标。底部「管理」按钮跳转 Settings 剪贴板 tab（完整批量管理）。Settings 剪贴板/识别记录管理页手动「加载更多」分页，行操作与浮窗一致；管理页改进：全选排除收藏条目（避免收藏被批量删除）、收藏按钮置操作栏末尾（与浮窗一致）、图片条目时间后显示存储大小（高亮 sky-700）、过滤栏加 OCR 类别（ScanText 图标，与浮窗一致）。修改命令（删除/清空/收藏/批量删除）成功后 `emit("clipboard://changed")` 广播，浮窗（`useClipboardHistory`）+ 设置页（`ClipboardPanel`）双端同步刷新。文件路径跨平台格式不同（Linux X11/Wayland = `file://` URI + 百分号编码；macOS/Windows = 已解码普通路径），显示（`formatFilePaths`）与打开（`open_file_item`）仅对 `file://` 开头解码，避免误伤含字面 `%XX` 的普通路径（Windows 剪贴板文件恒为已解码普通路径——clipboard-rs `win.rs` 走 `CF_HDROP`/`FileList` 返回普通 `C:\...` 路径，`file://` 分支不会触发，故无 `file:///C:/` 前导斜杠问题）。`open_file_item` 打开命令按平台：macOS `open` / Linux `xdg-open` / Windows `cmd /c start ""`（默认关联程序）；Windows **不用** `explorer <file>`——它只在资源管理器「定位并选中」文件、不调默认程序（旧实现用 explorer，已改 cmd start）。 |

**macOS 动态激活策略（Dock 图标显隐）：** 应用启动即 `Accessory` 模式（无 Dock 图标，纯托盘应用）。三个**常规窗口**（`settings_window` / `compact_editor_window` / `image_preview_window`）任一打开时切 `Regular`（设置窗口另经 `set_dock_icon()` 用 `objc2` 手动 `setApplicationIconImage`——release 裸二进制无 .app bundle，Tauri 仅 debug 自动设图标）。关窗 `Destroyed` 经 `activation.rs::restore_accessory_if_no_regular_window` 协调——**仅当三个常规窗口全无存活才切回 `Accessory`**，否则保持 `Regular`（避免 app 降级时 macOS 连带收掉其余常规窗口，曾致「关 CompactEditor 连带关 image_preview」）。`#[cfg(target_os = "macos")]` 条件编译，Windows / Linux 无此逻辑。
...
- **Transcript 段模型文本状态机**：识别文本状态由 `Transcript`（`crates/desktop/src/transcript.rs`）统一管理——内部用 `segments: Vec<Segment>`（每段 `{kind: Raw/Polished/Edited, text}`）作结构化真相源 + `caret_gap`（新语音生长缝隙，0..=segments.len()，==len 即末尾追加，默认零回归）+ `pending_delete: Option<(usize,usize)>`（选中替换待删范围，运行时态不入库）。`finish_text()` 段扁平化为唯一展示/落库/复制文本（派生，不另存）。引擎累积全量经 `apply_engine_full` 取尾部 delta 在 `caret_gap` 生长（VadSegmented 走 `append_segment`）；`set_caret`/`set_selection` 经 `split_at` 劈段定位。`Stage::Streaming`/`VadSegmented`/`WaitingCompletion` 各持 `transcript`。停止后 `Stage::Polishing`（最终润色中，持 `id`+`raw_text`）→ `Stage::Pasting`（持 `id`+`raw_text`+`polished_text`+`polish_status`）；入库的 `engine`/`engine_mode` 在过程入库的 raw 阶段已写，`Pasting` 不再持有。详见 [spec §1/§4/§11](superpowers/specs/2026-07-03-asr-cursor-insert-design.md)
...
- **结果窗光标定位 + 中间插入 + 选中替换**（[spec](superpowers/specs/2026-07-03-asr-cursor-insert-design.md) §1–§11）：
  ...
  - **选中替换**：拖选一段 → `invoke("set_selection", {start,end})` → `set_selection` 记录 `pending_delete` + 劈 `caret_gap` 到 start（**不立即删**，保留浏览器原生高亮）；首个非空 delta 经**两条入口**（`apply_engine_full` 流式 local / `append_segment` VadSegmented 离线 + cloud partial）到达时都 `delete_range` 真删 + `push_delta_at_caret` 插入（删旧换新）——两入口须**对称消费** `pending_delete`，漏任一即该类引擎失效（初版漏 `append_segment`，离线/cloud 引擎选中后只插不删、选中文本残留，9d4a654 修，详见 spec §11.7）。点别处 `set_caret` 清 `pending_delete` 取消。前端 `onMouseUp`（选区 mouseup 才完整）按 `isCollapsed` 分流（折叠→set_caret 中插；非折叠→set_selection）。
...
- `transcriptions` 表（schema v15，段模型）：识别历史，结构化真相源为 `segments`（段 JSON `[{kind:raw|polished|edited, text}]`）+ `text`（= `finish_text()` 扁平，供 search/clipboard/history 直读）；v15 已 DROP 旧三列 `raw_text`/`polished_text`/`edited_text`——信息全可从 segments/text 派生，且段模型润色是 Raw 段原地替换为 Polished、本就不保留原始 Raw 文本（旧 `raw_text` 列在润色后存的已是含 Polished 段的扁平，名不副实）。另有润色状态（`polish_status`：`off`/`done`/`failed`）+ 元数据（engine / engine_mode / created_at / char_count / duration_ms）
- **过程增量入库（schema v15）**：`transcriptions.id` = 识别开始毫秒时间戳（`INTEGER PRIMARY KEY`，应用写入，去 `AUTOINCREMENT`），兼任主键 / 业务 key / 开始时间戳；`duration_ms = finalize_now_ms - id`。入库时机分散到识别过程各事件，每次同步写 `segments`（真相源）+ `text`（扁平）两列：首次有 ASR 文本 → `INSERT`（`insert_transcription_at_id`，写 segments + text）；分段 / 流式 partial → `update_text_segments`（写 text + segments + char_count）；停顿润色完成 → `update_polished`（写 polish_status + polish_model + segments + text）；停止 → `finalize`（含 `duration_ms`，`finalize_transcription` 写 text + segments + polish_status + polish_model + char_count + duration_ms）。DB 失败仅 `warn` log 不阻塞识别（best-effort）。迁移链：v13→v14 ALTER 加 segments + text 列 + Rust 读旧三列（edited≻polished≻raw 优先级）映射为单段写回；v14→v15 DROP 三冗余列（信息已在 segments/text）。详见 [spec](superpowers/specs/2026-06-14-archived-design.md)
...
- **手编 `models` 表 / `app_config` 表需重启进程生效**（`OnceLock` 缓存，运行中不可热更新；运行时修改走 `RuntimeConfig` + `persist_*`）。DB schema `user_version` 当前为 16（迁移链 v12→v16 见上方 octopus-infra 节；其中 models 表 `category` 列由 v2→v3 ALTER TABLE 补）。
