...
9. **AI 命令面板**（2026-07-08）——`action_bar_window` 迷你浮窗，全局热键触发。选中文本→模拟 Cmd+C→弹出两级菜单（AI/翻译/搜索/网页 + AI 子菜单润色/摘要/解释 + 搜索子菜单 Google/百度/Bing）→AI 结果打开 CompactEditor 临时 tab 展示（不写 DB）。热键触发前隐藏常规窗口（防 Regular 激活策略把 settings 带到前台）。trigger 在后台线程执行（200ms sleep 不阻塞主线程）。`TRIGGER_IN_PROGRESS: AtomicBool` 重入 guard 防热键连按丢失窗口记录。所有出口经 `finalize_action_bar(app)` 统一收口（恢复常规窗口 + 重置 guard）。trigger 阶段 suppress watcher → Cmd+C → 读选中 → 立即 `write_text` 恢复原始剪贴板（选中文本存 `PENDING_CONTEXT` 变量，不入剪贴板历史库）。AI 动作走 `octopus_llm::chat_text_with_prompt(system, user, config)` 参数注入 prompt，不碰全局 `SYSTEM_PROMPT`（防污染并发 ASR 润色）。窗口 show 操作经 `app.run_on_main_thread()` 确保在主线程执行 NSWindow 操作。仅 macOS 支持（非 macOS `trigger_action_bar` 直接 return）。capabilities 白名单必须包含 `action_bar_window`（否则 listen/invoke 被拒）。
