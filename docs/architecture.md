...
- `models` 表：模型目录（**唯一来源**，schema v28，domain 支持 `'asr' | 'llm' | 'ocr' | 'translate'`）；`source`（`is_local=1`）= 路径标识（如 `asr/whisper-small`、`translate/opus-mt`、`ocr/PP-OCRv6-small`），相对于 `~/.octopus/models/`；`secret_key`（`is_local=1`）= **声明式下载清单 manifest JSON**（`{"<相对路径>": {"source", "sha256", "size}}`，source URL 支持 `{env.huggingface}` / `{env.github}` 模板变量），`secret_key`（`is_local=0`）= API Key；本地 ASR（is_local=1，13 行）+ 翻译（2 行）+ OCR（2 行）初始 manifest 由 `model_manifests.rs` 常量预填（从 HF cache 本地文件计算），首次下载后校验完整。唯一键 `UNIQUE(domain, provider, category, model_name)`
...
- `list_downloadable_models(domain)`：**v3 直读 DB** `list_local_models_by_domain(domain)`，按 `is_enabled` 显示就绪/下载。统一 asr/translate/ocr 三个 domain。
- `download_model(repo)`：**v3 manifest 驱动**——从 DB 读 `secret_key` 清单 JSON → 解析 `{env.*}` 模板变量 → 逐文件下载到 `~/.octopus/models/{source}/{path}` + SHA256 校验 → `is_enabled=true`。
- `verify_model`：完整性 sha256 复核，搜索所有 domain（asr/translate/ocr）。
