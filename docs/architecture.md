...
独立的记事本业务 crate，仅依赖 `octopus-infra`。定位为「内容收集箱」——ASR / OCR / 转译记录的结果一键存为新笔记。**三类型**（`NoteType`：`html` 富文本 TipTap / `text` 纯文本 / `markdown` md 源码），html 为默认主类型，md/txt/html 为序列化格式。序列化用 `scraper`（HTML→纯文本），文件 I/O 用 `std` + `dirs`。

| 模块 | 职责 |
|---|---|
| `model` | `NoteSource` 枚举（`asr`/`ocr`/`clipboard`/`manual`，serde 小写，manual 默认）+ `as_str`/`from_str`；`NoteType` 枚举（`html`/`text`/`markdown`，serde 小写，html 默认，`from_str` 未知→html 容错）+ `as_str`/`from_str`；`Note`（11 字段，含 `note_type`）；`NoteFilter` |
| `serialize` | `extract_text(html)`：scraper 解析 HTML → 纯文本（块级元素换行、`<img>`→「[图片]」、折叠空白），供 FTS 索引与导出 txt |
| `store` | CRUD + FTS5 检索 + 计数 + 排序（`is_pinned DESC, updated_at DESC, id DESC`）+ 分页 + `toggle_pinned`/`toggle_favorite`；`split_body(body, type)` 按类型分发——html→`(body, extract_text(body))`（content_html 存原始、content_text 存抽取），text/markdown→`("", body)`（content_text 直存原文/源码）；CJK 三元分词：≥3 字符走 MATCH 短语，<3 字符 LIKE 回退 |
...
**DB**：`notes` 表（`crates/infra/src/db.sql`，`id INTEGER PRIMARY KEY AUTOINCREMENT`——与 clipboard/transcriptions 毫秒戳 id 不同，notes 有独立 `created_at`/`updated_at` 列；字段 title/content_text/content_html/type/source/source_ref_id/is_pinned/is_favorite/created_at/updated_at；`type TEXT NOT NULL DEFAULT 'html'` 为内容格式）+ `notes_fts`（FTS5 `trigram` 分词，`content='notes'`）+ 3 触发器（insert/update/delete 同步 fts）。迁移经 `init_schema` 的 `PRAGMA user_version` v8→v9→v10→v11（`crates/infra/src/db.rs`；v9→v10 幂等 `ALTER ADD type`；**v10→v11 幂等 `ALTER ADD content_html`**——兼容 egui 分支重建过的库：egui 的 v10 丢了 content_html，致 webview `SELECT content_html` 报 no such column、记事本空白；补 `DEFAULT ''` 后历史 egui 笔记按 type=text 读 content_text 显示）。
...
- `note_commands.rs`：14 个 Tauri 命令薄层 + 2 个集成入口（`save_transcription_to_note` / `save_ocr_to_note`，写入即 `emit("notepad://changed")`）+ 图片桥接。`create_note`/`update_note` 透传 `note_type`（前端按 type 传 body+type）；`save_transcription_to_note`/`save_ocr_to_note` 固定 `type=text`（去 `<p>` 包裹直存原文）。（`save_clipboard_to_note` 已于 2026-07-01 移除——剪贴板条目不再存入记事本；`NoteSource::Clipboard` 变体保留供 DB 历史数据反序列化。）
...
- 前端：`pages/Notepad/` 按 `note_type` 分发编辑器——html=TipTap v3（工具栏 + 800ms 防抖自动保存 + 导入导出 + `note-img` 图片渲染）、text=textarea、markdown=`MarkdownEditor`（marked 预览 + 编辑切换）；新建笔记弹 type 选择菜单（已建锁定不可改），`NoteList` 行显示 T/MD 角标。`useNotes` hook（列表 + 搜索防抖 + 分页 + `notepad://changed` 监听）。
