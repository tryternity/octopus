...
- **设置页（命令面板 tab）**：CRUD + 排序。**2026-07-17 双 TAB + 左右分栏重构**——原递归 TreeNode 树形（chevron 展开/收起 + 全部展开/收缩按钮）改为：(1) 顶部 `UnderlineTabs` 切「命令管理」/「执行记录」；(2) 命令管理 TAB 左右分栏——左栏（w-64）主菜单列表（`MenuRow` 单选高亮 + 上移/下移/编辑/删除，场景过滤 Segmented，新增主菜单按钮），右栏顶部主菜单 inline 编辑表单（标题/类型/快捷键/全局快捷键/启用，onChange 实时保存 `updateMainInline`）+ 下方子菜单列表（仅 submenu 类型，叶子命令显示提示）；子菜单编辑仍走全屏 `EditForm`（editingId/draftParentId 判定覆盖）；`MenuRow` 主菜单/子菜单共用（替代原 TreeNode）。新增用内存草稿模式——不写 DB，保存时才 create，取消零脏数据；删除二次确认。详见 [spec §6.3](superpowers/specs/2026-07-09-action-bar-menu-db-design.md) + [双栏重构 spec](superpowers/specs/2026-07-17-action-bar-panel-dual-column.md) + [脚本增强 plan](superpowers/plans/2026-07-10-action-bar-script-enhancement.md) + [Extension Package plan](superpowers/plans/2026-07-10-extension-package.md)。
