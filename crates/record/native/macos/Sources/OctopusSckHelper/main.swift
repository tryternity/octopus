...
struct OctopusSckHelper {
	static func main() async {
		...
		// ── A2 spike：验证 SCK 私有 KVC 能否让 system+mic 合一条 .audio 流 ──
		// 临时分支（Phase 0 完成后删除）。详见 plan Task 0.1。
		if args.contains("--spike-single-stream") {
			guard #available(macOS 13.0, *) else {
				FileHandle.standardError.write("spike requires macOS 13+\n".data(using: .utf8)!)
				exit(1)
			}
			await SpikeSingleStream.run()
			exit(0)
		}
...
// MARK: - A2 spike（Phase 0 临时代码，spike 完成后删除）
//
// 验证 SCK 私有 KVC 能否让 system+mic 合并到单条 .audio 流。
// 详见 plan docs/superpowers/plans/2026-07-27-screen-record-audio-mix-redesign.md Task 0.1。
//
// 用法：
//   swift run -c release OctopusSckHelper --spike-single-stream
//
// 预期输出（A2 失败，转 A1）：
//   [spike] KVC audioMix = undefined ...
//   [spike] KVC combinedMicrophoneAudio = undefined ...
//   [spike] RESULT: .audio=N fmt=...
//   [spike] RESULT: mic(=2)=M fmt=...
//   [spike] DECISION: mic SEPARATE from .audio (A2 FAIL → A1)
...
enum SpikeSingleStream {
	static func run() async {
		...
		// P1: 当前 helper 同款 private KVC（让 mic 走 rawValue=2）
		configuration.setValue(true, forKey: "captureMicrophone")

		// P2: 试探其他可能合并 mic 到 .audio 的私有 KVC
		// 用 responds(to:) 做能力检测——setValue:forKey: 对未定义 key 会抛 ObjC
		// NSUndefinedKeyException，Swift 的 do-catch 接不住（会 crash）。
		let candidates = [
			"setAudioMix:", "setCombinedMicrophoneAudio:", "setMergeMicrophone:",
			"setMicrophoneIntoAudioStream:", "setMixMicrophoneIntoAudio:",
			"setIncludeMicrophoneInAudioCapture:", "setMicrophoneMix:",
		]
		for selectorString in candidates {
			let selector = Selector(selectorString)
			if configuration.responds(to: selector) {
				configuration.perform(selector, with: true as NSNumber)
				FileHandle.standardError.write("[spike] KVC \(selectorString) RESPONDS + set\n".data(using: .utf8)!)
			} else {
				FileHandle.standardError.write("[spike] KVC \(selectorString) not supported\n".data(using: .utf8)!)
			}
		}

		guard let content = try? await SCShareableContent.excludingDesktopWindows(false, onScreenWindowsOnly: true) else {
			FileHandle.standardError.write("[spike] no shareable content\n".data(using: .utf8)!)
			return
		}
		guard let display = content.displays.first else {
			FileHandle.standardError.write("[spike] no display\n".data(using: .utf8)!)
			return
		}
		...
		do {
			...
			FileHandle.standardError.write("[spike] startCapture failed: \(error)\n".data(using: .utf8)!)
			return
		}
		FileHandle.standardError.write("[spike] capturing 5s...\n".data(using: .utf8)!)
...
final class SpikeCounter: NSObject, SCStreamOutput {
	...
	func stream(_ stream: SCStream, didOutputSampleBuffer sampleBuffer: CMSampleBuffer, of type: SCStreamOutputType) {
		if type == .audio {
			audioCount += 1
			if audioCount == 1 { audioFormat = formatDescription(of: sampleBuffer) }
		}
		if type.rawValue == 2 {
			micCount += 1
			if micCount == 1 { micFormat = formatDescription(of: sampleBuffer) }
		}
	}

	private func formatDescription(of sb: CMSampleBuffer) -> String {
		...
		return "sr=\(asbd.mSampleRate) ch=\(asbd.mChannelsPerFrame) flags=0x\(String(asbd.mFormatFlags, radix: 16)) bytesPerFrame=\(asbd.mBytesPerFrame)"
	}

	func report() {
		FileHandle.standardError.write("[spike] RESULT: .audio count=\(audioCount) fmt=\(audioFormat)\n".data(using: .utf8)!)
		FileHandle.standardError.write("[spike] RESULT: mic(=2) count=\(micCount) fmt=\(micFormat)\n".data(using: .utf8)!)
		let decision = (micCount == 0 && audioCount > 0)
			? "mic MERGED into .audio (A2 SUCCESS)"
			: "mic SEPARATE from .audio (A2 FAIL → A1)"
		FileHandle.standardError.write("[spike] DECISION: \(decision)\n".data(using: .utf8)!)
	}
}
