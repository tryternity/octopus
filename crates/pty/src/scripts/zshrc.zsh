...
# command-done-with-exit-code) so the host can detect command boundaries,
# agent CLI invocations, and track cwd without re-parsing the prompt.
# `status` is a read-only special in zsh, so we shadow $? into `_octopus_ret`.
