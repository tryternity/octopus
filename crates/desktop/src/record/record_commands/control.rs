//! 录屏控制命令子模块（macOS 独占）。
//!
//! 从 record_commands/mod.rs 拆出（Task 1.4）。包含：
//! - RecordConfig（前端组装的录制配置）
//! - record_start / record_start_default / record_pause / record_resume / record_stop / record_kill
//! - build_default_config / start_with_config（hotkey/tray 复用）
//! - parse_bool_config / resolve_mic_device_name（DB 配置解析 + 麦克风设备名三级回退）
//! - stop_and_store / derive_fields_from_request / MetaFields / stop_and_store_inner（入库逻辑）
//! - 3 个 resolve_mic_device_name_* 测试
