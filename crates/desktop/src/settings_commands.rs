...
pub fn set_config(
    ...
    let (old_asr_sc, old_clipboard_sc, old_edit_global, old_screenshot_sc, mut cfg) = {
        let g = rc.read().unwrap();
        (g.asr_shortcut.clone(), g.clipboard_shortcut.clone(), g.edit_global_shortcut.clone(), g.screenshot_shortcut.clone(), g.clone())
    };
