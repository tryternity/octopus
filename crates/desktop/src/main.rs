...
/// 原生 NSView 滚动截屏（独立 crate 验证）
#[tauri::command]
fn scroll_capture_start(app_handle: tauri::AppHandle) {
    ...
    scroll_capture::start(Box::new(move |png_bytes| {
        ...
        // 写系统剪贴板
        if let Some(handle) = ah.try_state::<std::sync::Arc<octopus_clipboard::ClipboardHandle>>() {
