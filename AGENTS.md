...
- **WKWebView 的 `window.confirm`/`alert` 不可靠（同步 JS 对话框在 WKWebView 静默失败）——确认框用 `tauri-plugin-dialog` 的 `confirm`，提示用自绘 toast（`useToast`）替代（2026-08-03 全量迁移完成，勿在新代码用原生 confirm/alert）**
